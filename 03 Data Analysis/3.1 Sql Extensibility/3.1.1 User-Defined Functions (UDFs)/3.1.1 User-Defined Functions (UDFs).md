...
```mermaid
flowchart TD
    A[Query Compiler] --> B[Compilation & Resolution]
    B --> B1[Validate UDF signature]
    B1 --> B2[Resolve data types]
    B2 --> B3[Check volatility / security context]
    B3 --> C[Parameter Binding per row]

    C --> D{NULL on NULL input?}
    D -- Yes --> E[Short-circuit: return NULL]
    D -- No --> F[Handler Invocation]

    F --> G{Type of UDF}
    G -- SQL UDF --> H[Evaluate logic inline]
    G -- Scripting UDF --> I["Batch of rows to runtime handler\n(vectorized or row-wise)"]
    G -- External UDF --> J["Serialize rows, batch, and send over HTTPS"]
    ...
    K --> L[Return typed values]
    L --> M{Is it a UDTF?}
    M -- Yes --> N["May return multiple rows\nper input row"]
    M -- No --> O[Return single value per row]

    N --> P[Pipeline Continuation]
    O --> P

    P --> Q["Feed results into subsequent filters,\njoins, or aggregations"]
    Q --> R{Evaluate result cache eligibility\nbased on volatility flag}

    R --> S[End of pipeline step]

    subgraph Notes
        T["Row count: stable for scalar UDFs\nUDTFs expand cardinality based on logic"]
        U["State does NOT persist across\nmicro-partitions or query executions"]
    end
...
```mermaid
flowchart TD
    A["Business Logic (Execution Logic)"] --> B["Volatility Semantics"]

    B --> B1["IMMUTABLE: Same output for same inputs\nEligible for result caching & compile-time optimization"]
    B --> B2["STABLE: Same output for same inputs within a single statement\nNot cacheable across queries"]
    B --> B3["VOLATILE (default for time/random)\nMay return different outputs\nDisables result caching"]

    A --> C["Security Context"]
    C --> C1["SECURITY DEFINER (default)\nExecutes with creator's privileges"]
    C --> C2["SECURITY INVOKER\nExecutes with caller's privileges"]
    C2 --> C3["Exam trap: INVOKER requires caller\nto hold privileges on all objects referenced inside the UDF"]

    A --> D["Statelessness Constraint"]
    D --> D1["Cannot write to tables\nCannot call stored procedures\nCannot maintain session state"]
    D1 --> D2["Persistent logic requires external services\nor Snowflake Streams/Tasks"]

    A --> E["Execution Parallelism"]
    E --> E1["Executes across all warehouse nodes and partitions\nResource consumption scales linearly with row count & handler complexity"]

    A --> F["Exam Traps - Key Distinction"]
    F --> F1["UDFs are NOT stored procedures"]
    F1 --> F2["Cannot perform DDL/DML\nCannot manage transactions\nCannot use CALL semantics"]
    F2 --> F3["Misclassification is a frequent exam error"]

    style A fill:#4a90e2,color:#fff
    style F fill:#e67e22,color:#fff
    style C3 fill:#f39c12,color:#000
    style F3 fill:#f39c12,color:#000
```
...
```mermaid
flowchart TD
    A["Query History"] --> A1["Columns: COMPILATION_TIME, EXECUTION_TIME\nErrors: ERROR_CODE, ERROR_MESSAGE"]

    B["Cache Hit Rate"] --> B1["Check RESULT_REUSED column in QUERY_HISTORY\nVOLATILE UDFs consistently show FALSE"]

    C["Resource Monitoring"] --> C1["Warehouse metrics spike for CPU-heavy UDFs:\nACTIVE_RUN_STATEMENT_COUNT, AVG_RUNNING_PERCENTAGE"]
    C1 --> C2["Python/Java sandboxes log initialization overhead"]

    D["External Function Telemetry"] --> D1["API gateway logs: payload size, latency, HTTP status"]
    D1 --> D2["Network rule violations appear as\nNETWORK_RULE_ACCESS_DENIED"]
...
```mermaid
flowchart TD
    Q1["Which execution mode?"] --> R1["Row-wise (row-by-row)"]
    Q1 --> R2["Vectorized / Batch\n(Python/Java handlers)"]

    R1 --> O1["High interpreter overhead per row\nSuitable for small row counts\nLower memory footprint"]
    R2 --> O2["5-10x lower overhead\nHigher memory pressure\nSuitable for large row counts\nMonitor memory limits"]
```
...
```mermaid
flowchart TD
    Q2["Is the UDF deterministic?"] -->|Yes| D1["Mark as IMMUTABLE"]
    Q2 -->|No or VOLATILE| D2["Result cache disabled\nRepeated executions consume full compute"]

    D1 --> O3["Enables result caching\nReduces compute cost"]
```
...
```mermaid
flowchart TD
    Q3["Does WHERE clause wrap a clustering key inside a UDF?"] -->|Yes| D3["Micro-partition pruning is prevented"]
    Q3 -->|No| D4["Pruning works normally"]

    D3 --> O4["Rewrite:\nWHERE cluster_key = value\nand apply UDF in SELECT projection"]
```
...
```mermaid
flowchart TD
    Q4["What is the primary bottleneck?"] -->|CPU-bound| D5["Use multi-cluster warehouses\n(more clusters, not larger nodes)"]
    Q4 -->|Memory-bound| D6["Increase node size (LARGE+)\nAccommodates sandbox overhead"]

    D5 --> O5["Horizontal scaling\nHandles concurrency & CPU load"]
    D6 --> O6["Vertical scaling\nLarger memory per node"]
```
...
```mermaid
flowchart TD
    Q5["What dominates latency?"] --> N1["Network round-trip &\nserialization costs"]

    N1 --> D7["Adjust batch size\n(default ~16KB per row, configurable)"]
    D7 --> D8["Monitor rate limits & payload size"]
    D8 --> O7["Optimize batch size to balance\nthroughput vs. throttling risk"]
```
