...
```mermaid
flowchart TD
    A[CALL_ASYNC invocation] --> B{Privilege Validation}
    B --> |Caller must have| C[USAGE on target warehouse<br>AND EXECUTE on stored procedure]
    C --> |Missing any| D[Immediate failure<br>No background session starts]
    C --> |Valid| E[Background session created<br>with caller's role]

    E --> F[Role Inheritance]
    F --> G[Session executes as caller's role<br>No privilege escalation]
    G --> H{Caller has required object<br>privileges? e.g., SELECT}
    H --> |No| I[Execution fails.<br>Error captured in status metadata]
    H --> |Yes| J[Proceed]

    J --> K[Data Isolation]
    K --> L[Background session cannot access:<br>• Caller's temporary objects<br>• Session-specific variables]
    L --> M[Row access policies & masking policies<br>evaluate normally within async context]

    M --> N[Exam Note]
    N --> O[Async execution does NOT bypass security boundaries.<br>All object access validated against inherited role.<br>Failed privilege checks captured in status metadata,<br>not caller session.]

    style A fill:#f9f,stroke:#333,stroke-width:2px
    style D fill:#f66,color:white
