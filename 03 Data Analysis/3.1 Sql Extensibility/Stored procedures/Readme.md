...
flowchart TD
    ...
    Q2 --> |"Session-level needed"| A2["Access via CURRENT_SESSION() or explicit parameter passing"]

    A1 --> Q3{"Cursor or RESULTSET"}
    A2 --> Q3

    Q3 --> |Created| C1["Cursor or RESULTSET opened"]
    C1 --> Q4{"Explicitly consumed or closed?"}
    Q4 --> |Yes| C2["Resources released promptly"]
    Q4 --> |No| C3["Resources released at procedure termination, may impact intermediate memory"]

    C2 --> Q5{"Transaction Type"}
    C3 --> Q5

    Q5 --> |DML| T1{"Explicit transaction block? Snowflake Scripting"}
    T1 --> |Yes| T2["Can COMMIT or ROLLBACK within block"]
    T1 --> |No| T3["Auto-commit per statement, default for external languages"]

    Q5 --> |DDL| T4["Auto-commit immediately, invalidates any active transaction"]
    ...
    N1 --> |"CALLED ON NULL INPUT"| N2["Procedure executes, parameter value is NULL"]
    N1 --> |"RETURNS NULL ON NULL INPUT"| N3["Procedure returns NULL immediately without executing body"]
    N1 --> |"Default (CALLED ON NULL INPUT)"| N2

    Q6 --> |"Standard SQL null propagation"| N4["Nulls propagate through expressions. Procedure must explicitly handle nulls if not using special clauses"]
