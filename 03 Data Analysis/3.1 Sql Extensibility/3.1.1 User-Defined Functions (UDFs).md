...
flowchart TD
    ...
    G -- Scripting UDF --> I[Batch of rows to runtime handler<br/>(vectorized or row-wise)]
    ...
    L --> M{Is it a UDTF?}
    M -- Yes --> N[May return multiple rows per input row]
    M -- No --> O[Return single value per row]

    N --> P[Pipeline Continuation]
    O --> P

    P --> Q[Feed results into subsequent filters,<br/>joins, or aggregations]
    Q --> R{Evaluate result cache eligibility<br/>based on volatility flag}

    R --> S[End of pipeline step]

    subgraph Notes
        T[Row count: stable for scalar UDFs<br/>UDTFs expand cardinality based on logic]
        U[State does NOT persist across<br/>micro-partitions or query executions]
    end
