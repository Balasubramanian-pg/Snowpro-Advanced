...
| :--- | :--- | :--- | :--- | :--- |
| [`USE <object>`](Command Pattern Summary/USE object.md) | Context Setter | Modifies the active session namespace or compute context. | Cloud Services | Success message; updated session state. |
| [`SHOW <objects>`](Command Pattern Summary/SHOW objects.md) | Metadata List | Retrieves a list of objects and their high-level metadata (rows, size, owner). | Cloud Services | Tabular result set (1 row per object). |
| [`DESCRIBE <object>`](Command Pattern Summary/DESCRIBE object.md) | Schema Inspection | Retrieves the structural definition, data types, and defaults of a specific object. | Cloud Services | Tabular result set (1 row per column/property). |
