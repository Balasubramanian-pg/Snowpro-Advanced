...
| :--- | :--- | :--- | :--- | :--- |
| [**Join**](SQL Object Summary/Join.md) | `INNER`, `LEFT/RIGHT/FULL OUTER` | Matches rows based on key predicates. | Multiplies, preserves, or drops rows based on match cardinality. | `Hash Join` (Typically) |
| [**Join**](SQL Object Summary/Join.md) | `CROSS JOIN` | Produces a Cartesian product of both tables. | Rows A × Rows B. | `Cartesian Join` / `Nested Loop` |
| [**Join**](SQL Object Summary/Join.md) | `ASOF JOIN` | Matches time-series data to the closest prior or exact timestamp. | Single nearest match per row. | `ASOF Join` |
| [**Set**](SQL Object Summary/Set.md) | `UNION ALL` | Appends rows from Query B to Query A. | Rows A + Rows B. | `UnionAll` |
| [**Set**](SQL Object Summary/Set.md) | `UNION` | Appends rows and removes all duplicates. | (Rows A + Rows B) - Duplicates. | `UnionAll` followed by `Aggregate` |
| [**Set**](SQL Object Summary/Set.md) | `INTERSECT` | Returns only distinct rows present in both queries. | Overlapping distinct rows. | `Aggregate` |
| [**Set**](SQL Object Summary/Set.md) | `MINUS` / `EXCEPT` | Returns distinct rows from Query A absent in Query B. | Distinct Rows A - Overlap. | `Aggregate` |
