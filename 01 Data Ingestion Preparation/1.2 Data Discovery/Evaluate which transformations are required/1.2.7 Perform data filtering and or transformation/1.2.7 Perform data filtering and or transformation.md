...
| :--- | :--- | :--- | :--- | :--- | :--- |
| [`WHERE`](SQL Object  Feature Summary/WHERE.md) | Filter | Eliminates rows based on base column predicates. | Raw rows | Pruned rows | Pre-aggregation |
| [`HAVING`](SQL Object  Feature Summary/HAVING.md) | Filter | Eliminates rows based on aggregated values. | Grouped rows | Pruned grouped rows | Post-aggregation |
| [`QUALIFY`](SQL Object  Feature Summary/QUALIFY.md) | Filter | Eliminates rows based on window function results. | Windowed rows | Pruned windowed rows | Post-windowing |
| [`CAST` / `TRY_CAST`](SQL Object  Feature Summary/CAST  TRY_CAST.md)| Transformation | Coerces data types (e.g., String to Timestamp). | Raw column | Strongly-typed column | Row evaluation |
| [`CASE` / `IFF`](SQL Object  Feature Summary/CASE  IFF.md) | Transformation | Applies conditional logic to derive new values. | Multiple columns | Derived scalar value | Row evaluation |
| [`COPY INTO ... SELECT`](SQL Object  Feature Summary/COPY INTO ... SELECT.md) | Ingestion Pipeline | Applies lightweight filtering/transformation during file load. | External/Internal Stage | Target Table | Load time |
...
| :--- | :--- | :--- | :--- |
| [`TIMESTAMP_INPUT_FORMAT`](Parameters  Configuration/TIMESTAMP_INPUT_FORMAT.md) | String | Defines how strings are parsed into timestamps during `CAST` or `COPY INTO`. | `AUTO` |
| [`DATE_OUTPUT_FORMAT`](Parameters  Configuration/DATE_OUTPUT_FORMAT.md) | String | Defines how dates are transformed into strings for final projection. | `YYYY-MM-DD` |
| [`ERROR_ON_NONDETERMINISTIC_MERGE`](Parameters  Configuration/ERROR_ON_NONDETERMINISTIC_MERGE.md) | Boolean | Forces a `MERGE` transform to fail if multiple source rows match a single target row. | `TRUE` |
