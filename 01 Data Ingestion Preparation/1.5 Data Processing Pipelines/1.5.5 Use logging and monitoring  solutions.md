...
|---|---|---|---|---|---|
| INFORMATION_SCHEMA.QUERY_HISTORY | System view | Near-real-time query telemetry | Query execution engine | Query text, duration, bytes scanned, warehouse | Query-time |
| ACCOUNT_USAGE.QUERY_HISTORY | Account view | Long-term query audit trail | Query execution engine | Same as above, 365-day retention | Query-time, 45-min latency |
| INFORMATION_SCHEMA.TASK_HISTORY | System view | Near-real-time task telemetry | Task scheduler | State, error code, timing, query ID | Query-time |
| ACCOUNT_USAGE.TASK_HISTORY | Account view | Long-term task audit trail | Task scheduler | Same as above, 365-day retention | Query-time, 45-min latency |
| ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY | Account view | Credit consumption by warehouse | Warehouse usage | Credits used, start/end time | Query-time, 45-min latency |
| ACCOUNT_USAGE.PIPE_USAGE_HISTORY | Account view | Snowpipe load telemetry | Pipe operations | Files loaded, credits used, status | Query-time, 45-min latency |
| ACCOUNT_USAGE.LOGIN_HISTORY | Account view | Authentication events | Identity layer | User, IP, status, timestamp | Query-time, 45-min latency |
| RESOURCE MONITOR | Account object | Credit threshold enforcement | Warehouse consumption | Alert or suspend action | Automatic at threshold |
| NOTIFICATION INTEGRATION | Account object | External alert routing | Resource monitor or task state | Payload to email/Slack/webhook | Automatic on trigger |
| QUERY_TAG | Session parameter | Execution traceability | User-defined string | Tagged queries in history views | Per-session or per-query |
| DATA METRIC FUNCTION | Schema object | Programmatic data quality check | Table column or expression | Pass/fail result in `DATA_METRIC_FUNCTION_REFERENCES` | Automatic or scheduled |
| STREAM object metadata | System view | Stream health and staleness | Stream state | `STALE` flag, source table | Query-time |
...
| `ERROR_CODE` | Failure classification | One per failed query | Null on success |
| `ERROR_MESSAGE` | Failure detail | One per failed query | Null on success |
| `START_TIME` | Timing | One per query | |
| `END_TIME` | Timing | One per query | |
| `TOTAL_ELAPSED_TIME` | Duration ms | One per query | |
| `BYTES_SCANNED` | Data volume | One per query | Micro-partition bytes |
| `ROWS_PRODUCED` | Result volume | One per query | |
| `CREDITS_USED_CLOUD_SERVICES` | Cost | One per query | Cloud services credits |
...
| `CREDITS_USED` | Cost | One per warehouse per interval | Includes cloud services allocation |
| `CREDITS_USED_COMPUTE` | Compute cost | One per warehouse per interval | Warehouse compute only |
...
| Column | Role | Grain | Notes |
|---|---|---|---|
| `NAME` | Task identifier | One per execution | |
| `QUERY_ID` | Execution trace | One per execution | Joins to `QUERY_HISTORY` |
| `DATABASE_NAME` | Context | One per execution | |
| `SCHEMA_NAME` | Context | One per execution | |
| `SCHEDULED_TIME` | Intended start | One per execution | |
| `COMPLETED_TIME` | Actual end | One per execution | |
| `STATE` | Outcome | One per execution | `SUCCEEDED`, `FAILED`, `SKIPPED`, `CANCELLED` |
| `ERROR_CODE` | Failure code | One per failed execution | |
| `ERROR_MESSAGE` | Failure detail | One per failed execution | |
| `RUN_ID` | Instance ID | One per execution | |
...
|---|---|---|---|
| `CREDIT_QUOTA` | Threshold | Number of credits | None |
| `FREQUENCY` | Evaluation window | `MONTHLY`, `DAILY`, `WEEKLY`, `YEARLY`, `NEVER` | `MONTHLY` |
| `START_TIMESTAMP` | Window anchor | Timestamp | Current time |
| `TRIGGERS` | Action bindings | `ON {PERCENT} PERCENT DO {SUSPEND \| SUSPEND_IMMEDIATE \| NOTIFY}` | None |
...
|---|---|---|---|---|---|---|
| `QUERY_TAG` | Session parameter | Execution traceability | String <= 256 chars | None | Session or query | Tags history records |
| `TIMESTAMP_OUTPUT_FORMAT` | Session parameter | Timestamp display format | Format string | `YYYY-MM-DD HH24:MI:SS.FF3 TZHTZM` | Session | Affects timestamp rendering in results |
| `TIMEZONE` | Session parameter | Timestamp context | IANA timezone | `UTC` | Session | Affects `CURRENT_TIMESTAMP` and history timestamps |
| `WAREHOUSE` | Session parameter | Default compute | Warehouse name | None | Session | Determines where queries execute |
| `STATEMENT_TIMEOUT_IN_SECONDS` | Session parameter | Query abort threshold | Integer | `172800` (48 hours) | Session | Cancels long-running queries |
| `CREDIT_QUOTA` | Resource monitor | Spending limit | Number | None | `CREATE RESOURCE MONITOR` | Threshold for alert/suspend |
| `FREQUENCY` | Resource monitor | Evaluation period | `MONTHLY`, `DAILY`, `WEEKLY`, `YEARLY`, `NEVER` | `MONTHLY` | `CREATE RESOURCE MONITOR` | Reset window for quota |
| `TRIGGERS` | Resource monitor | Action bindings | Percent + action pairs | None | `CREATE RESOURCE MONITOR` | Defines alert thresholds |
| `NOTIFICATION_INTEGRATION` | Account object | Alert endpoint | Integration name | None | Resource monitor, Task | Routes alerts externally |
| `DATA_METRIC_SCHEDULE` | Table property | Quality check cadence | `USING CRON ...` or interval | On DML | `ALTER TABLE` | When metrics evaluate |
...
|---|---|---|
| Query ACCOUNT_USAGE | `MONITOR` or `ACCOUNTADMIN` | Account |
| Query INFORMATION_SCHEMA | `USAGE` on database/schema | Database/Schema |
| Create resource monitor | `CREATE RESOURCE MONITOR` | Account |
| Create notification integration | `CREATE INTEGRATION` | Account |
| View task history | `MONITOR` or `OWNERSHIP` | Task |
| Bind data metric function | `OWNERSHIP` on table | Table |
| Execute data metric function | `EXECUTE DATA METRIC FUNCTION` | Function |
