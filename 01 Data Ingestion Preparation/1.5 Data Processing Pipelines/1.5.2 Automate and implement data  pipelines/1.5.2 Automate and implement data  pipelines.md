...
|---|---|---|---|---|---|
| [Task](SQL Object Summary/Task.md) | Schema object | Schedules or chains SQL/procedure execution | SQL statement, stored procedure call, predecessor task state | Executed DML/DDL, task history entry | Time-based cron or predecessor trigger |
| [Stream](SQL Object Summary/Stream.md) | Schema object | Captures row-level changes on source object | DML on table, view, or external table | Delta rows with metadata action flags | Automatic, transaction-scoped |
| [Stored Procedure](SQL Object Summary/Stored Procedure.md) | Schema object | Encapsulates imperative or dynamic SQL logic | Input arguments, session context | Result set, output parameters, side effects | `CALL` from task, SQL, or API |
| [Pipe](SQL Object Summary/Pipe.md) | Schema object | Automates file ingestion from external stage | Cloud storage events or REST call | Loaded rows in target table | Snowpipe serverless or pipe REST API |
| [Error Integration](SQL Object Summary/Error Integration.md) | Notification object | Routes task failure/success alerts | Task state transitions | Notification payload to external endpoint | Automatic on task completion/failure |
| [Task DAG / Tree](SQL Object Summary/Task DAG  Tree.md) | Dependency graph | Defines execution order across pipeline stages | Root task schedule, child task definitions | Serialized or parallel task execution | Implicit via `AFTER` clause |
...
|---|---|---|---|
| [`NAME`](INFORMATION_SCHEMA.TASK_HISTORY/NAME.md) | Task identifier | One per task execution | Includes schema-qualified name |
| [`DATABASE_NAME`](INFORMATION_SCHEMA.TASK_HISTORY/DATABASE_NAME.md) | Context | One per task | Task database |
| [`SCHEMA_NAME`](INFORMATION_SCHEMA.TASK_HISTORY/SCHEMA_NAME.md) | Context | One per task | Task schema |
| [`QUERY_ID`](INFORMATION_SCHEMA.TASK_HISTORY/QUERY_ID.md) | Execution trace | One per task run | Joins to `QUERY_HISTORY` |
| [`CONDITION_TEXT`](INFORMATION_SCHEMA.TASK_HISTORY/CONDITION_TEXT.md) | WHEN clause | One per task definition | Stream condition or boolean |
| [`STATE`](INFORMATION_SCHEMA.TASK_HISTORY/STATE.md) | Outcome | One per run | `SCHEDULED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `SKIPPED` |
| [`ERROR_CODE`](INFORMATION_SCHEMA.TASK_HISTORY/ERROR_CODE.md) | Failure classification | One per failed run | Null if succeeded |
| [`ERROR_MESSAGE`](INFORMATION_SCHEMA.TASK_HISTORY/ERROR_MESSAGE.md) | Failure detail | One per failed run | Null if succeeded |
| [`SCHEDULED_TIME`](INFORMATION_SCHEMA.TASK_HISTORY/SCHEDULED_TIME.md) | Intended start | One per run | Cron-derived or predecessor-derived |
| [`COMPLETED_TIME`](INFORMATION_SCHEMA.TASK_HISTORY/COMPLETED_TIME.md) | Actual end | One per run | Used for duration calculation |
| [`RUN_ID`](Target Audit Table (Recommended Pattern)/RUN_ID.md) | Instance identifier | One per run | Unique within task history |
...
| Column | Role | Notes |
|---|---|---|
| [`STREAM_NAME`](INFORMATION_SCHEMA.STREAMS/STREAM_NAME.md) | Identifier | |
| [`TABLE_NAME`](INFORMATION_SCHEMA.STREAMS/TABLE_NAME.md) | Source object | Table, view, or external table |
| [`TABLE_DATABASE`](INFORMATION_SCHEMA.STREAMS/TABLE_DATABASE.md) | Source context | |
| [`TABLE_SCHEMA`](INFORMATION_SCHEMA.STREAMS/TABLE_SCHEMA.md) | Source context | |
| [`STALE`](INFORMATION_SCHEMA.STREAMS/STALE.md) | Offset status | `TRUE` if source DDL changed |
| [`OWNER`](INFORMATION_SCHEMA.STREAMS/OWNER.md) | Privilege context | |

## Target Audit Table (Recommended Pattern)

| Column | Role | Grain | Source |
|---|---|---|---|
| [`SURROGATE_KEY`](Target Audit Table (Recommended Pattern)/SURROGATE_KEY.md) | Primary key | One per record | Hash or sequence |
| [`BUSINESS_KEY`](Target Audit Table (Recommended Pattern)/BUSINESS_KEY.md) | Natural key | One per entity | Source system |
| [`PIPELINE_NAME`](Target Audit Table (Recommended Pattern)/PIPELINE_NAME.md) | Traceability | One per record | Task name or procedure |
| [`RUN_ID`](Target Audit Table (Recommended Pattern)/RUN_ID.md) | Execution trace | One per record | Task run or session |
| [`LOAD_TIMESTAMP`](Target Audit Table (Recommended Pattern)/LOAD_TIMESTAMP.md) | Audit time | One per record | `CURRENT_TIMESTAMP()` |
| [`SOURCE_STREAM`](Target Audit Table (Recommended Pattern)/SOURCE_STREAM.md) | Provenance | One per record | Stream or table name |
...
|---|---|---|---|---|---|---|
| [`WAREHOUSE`](Parameters  Variables  Configuration/WAREHOUSE.md) | Task property | Compute for task | Warehouse name or omitted for serverless | None (serverless if omitted) | `CREATE TASK` | Determines execution compute and cost model |
| [`SCHEDULE`](Parameters  Variables  Configuration/SCHEDULE.md) | Task property | Execution cadence | `USING CRON '...'` or `N MINUTES` | None (must specify or use AFTER) | `CREATE TASK` | Defines trigger timing for root tasks |
| [`AFTER`](Parameters  Variables  Configuration/AFTER.md) | Task property | Predecessor dependency | Comma-separated task names | None | `CREATE TASK` | Creates DAG edges |
| [`WHEN`](Parameters  Variables  Configuration/WHEN.md) | Task property | Conditional execution | Boolean expression | None (always run) | `CREATE TASK` | Skips execution if false |
| [`ERROR_INTEGRATION`](Parameters  Variables  Configuration/ERROR_INTEGRATION.md) | Task property | Notification target | Notification integration name | None | `CREATE TASK` | Routes failure alerts |
| [`USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE`](Parameters  Variables  Configuration/USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE.md) | Task property | Serverless task sizing | `XSMALL`, `SMALL`, `MEDIUM`, `LARGE`, etc. | `XSMALL` | `CREATE TASK` | Determines serverless compute allocation |
| [`USER_TASK_TIMEOUT_MS`](Parameters  Variables  Configuration/USER_TASK_TIMEOUT_MS.md) | Account parameter | Max task runtime | Milliseconds | 3600000 (1 hour) | Account | Aborts long-running tasks |
| [`SUSPEND_TASK_AFTER_NUM_FAILURES`](Parameters  Variables  Configuration/SUSPEND_TASK_AFTER_NUM_FAILURES.md) | Task property | Auto-suspend threshold | Integer >= 0 | 10 | `CREATE TASK` | Suspends task after N consecutive failures |
| [`COPY_OPTIONS`](Parameters  Variables  Configuration/COPY_OPTIONS.md) | Pipe property | Load behavior | `ON_ERROR`, `SIZE_LIMIT`, `PURGE`, etc. | Varies | `CREATE PIPE` | Controls file ingestion handling |
| [`COMMENT`](Parameters  Variables  Configuration/COMMENT.md) | Object property | Documentation | String | None | Task, Stream, Pipe | Metadata for operational context |
| [`ENABLE`](Parameters  Variables  Configuration/ENABLE.md) | Task state | Active/inactive | `TRUE` (resume), `FALSE` (suspend) | `FALSE` (suspended on creation) | `CREATE/ALTER TASK` | New tasks are suspended by default |
...
|---|---|---|
| [Create task](Privilege Requirements/Create task.md) | `CREATE TASK` | Schema |
| [Execute task (resume)](Privilege Requirements/Execute task (resume).md) | `EXECUTE TASK` | Account (for non-owners) |
| [Operate task](Privilege Requirements/Operate task.md) | `OPERATE` | Task |
| [Use warehouse](Privilege Requirements/Use warehouse.md) | `USAGE` | Warehouse |
| [Read stream](Privilege Requirements/Read stream.md) | `SELECT` | Source table/stream |
| [Modify target](Privilege Requirements/Modify target.md) | `INSERT`, `UPDATE`, `DELETE` | Target table |
| [Create pipe](Privilege Requirements/Create pipe.md) | `CREATE PIPE` | Schema |
| [Call procedure](Privilege Requirements/Call procedure.md) | `USAGE` | Procedure |
