...
|---|---|---|---|---|---|
| [Error Integration](SQL Object Summary/Error Integration.md) | Notification object | Routes task failure/success alerts to external systems | Task state transition | JSON payload to email/Slack/PagerDuty | Automatic on task completion |
| [Notification Integration](SQL Object Summary/Notification Integration.md) | Account object | Configures external endpoint for alerts | Task or pipe state | HTTP/HTTPS payload or email | Automatic on bound event |
| [TASK_HISTORY](SQL Object Summary/TASK_HISTORY.md) | System view | Records task execution outcomes | Task scheduler | Execution state, error codes, timing | Query-time |
| [STREAM staleness flag](SQL Object Summary/STREAM staleness flag.md) | Metadata property | Indicates source DDL changed | Source table DDL | `STALE = TRUE` in `INFORMATION_SCHEMA.STREAMS` | Automatic |
| [Quarantine Table](SQL Object Summary/Quarantine Table.md) | User table | Isolates failed or suspect rows | Pipeline validation logic | Rows failing quality checks | DML within task or procedure |
| [Error Log Table](SQL Object Summary/Error Log Table.md) | User table | Captures structured error context | Stored procedure catch blocks | Error code, message, run ID, timestamp | DML within procedure |
| [Retry Wrapper Procedure](SQL Object Summary/Retry Wrapper Procedure.md) | Stored procedure | Encapsulates idempotent retry logic | Task SQL or CALL | Success or terminal failure after N attempts | Called by task |
| [VALIDATION_MODE](SQL Object Summary/VALIDATION_MODE.md) | COPY option | Pre-validates files without loading | Stage files | Error report without persisted data | `COPY INTO` option |
| [PIPE_USAGE_HISTORY](SQL Object Summary/PIPE_USAGE_HISTORY.md) | Account view | Tracks pipe load outcomes | Snowpipe operations | File load status, error counts | Query-time |
...
|---|---|---|---|
| [`QUARANTINE_ID`](Quarantine Table (Recommended Pattern)/QUARANTINE_ID.md) | Surrogate key | One per rejected row | Auto-increment or sequence |
| [`SOURCE_TABLE`](Quarantine Table (Recommended Pattern)/SOURCE_TABLE.md) | Provenance | One per row | Origin table or stream |
| [`PIPELINE_NAME`](Quarantine Table (Recommended Pattern)/PIPELINE_NAME.md) | Context | One per row | Task or procedure name |
| [`RUN_ID`](Quarantine Table (Recommended Pattern)/RUN_ID.md) | Execution trace | One per row | Task run or session ID |
| [`REJECTED_ROW`](Quarantine Table (Recommended Pattern)/REJECTED_ROW.md) | Raw data | One per row | VARIANT or string representation |
| [`ERROR_CODE`](Error Log Table (Recommended Pattern)/ERROR_CODE.md) | Classification | One per row | SQL error code or custom code |
| [`ERROR_MESSAGE`](Error Log Table (Recommended Pattern)/ERROR_MESSAGE.md) | Detail | One per row | Full error or validation reason |
| [`REJECTED_TIMESTAMP`](Quarantine Table (Recommended Pattern)/REJECTED_TIMESTAMP.md) | Audit time | One per row | `CURRENT_TIMESTAMP()` |
| [`RESOLVED_TIMESTAMP`](Quarantine Table (Recommended Pattern)/RESOLVED_TIMESTAMP.md) | Resolution time | One per row | Null until fixed |
| [`RESOLUTION_ACTION`](Quarantine Table (Recommended Pattern)/RESOLUTION_ACTION.md) | Disposition | One per row | `REPROCESSED`, `ARCHIVED`, `DISCARDED` |
...
| Column | Role | Grain | Notes |
|---|---|---|---|
| [`LOG_ID`](Error Log Table (Recommended Pattern)/LOG_ID.md) | Surrogate key | One per error | |
| [`TASK_NAME`](Error Log Table (Recommended Pattern)/TASK_NAME.md) | Context | One per error | |
| [`QUERY_ID`](Error Log Table (Recommended Pattern)/QUERY_ID.md) | Traceability | One per error | Joins to `QUERY_HISTORY` |
| [`ERROR_CODE`](Error Log Table (Recommended Pattern)/ERROR_CODE.md) | Classification | One per error | |
| [`ERROR_MESSAGE`](Error Log Table (Recommended Pattern)/ERROR_MESSAGE.md) | Detail | One per error | Truncated if necessary |
| [`PROCEDURE_STATE`](Error Log Table (Recommended Pattern)/PROCEDURE_STATE.md) | Debug context | One per error | JSON of variable state |
| [`LOGGED_AT`](Error Log Table (Recommended Pattern)/LOGGED_AT.md) | Timestamp | One per error | |
| [`SEVERITY`](Error Log Table (Recommended Pattern)/SEVERITY.md) | Priority | One per error | `ERROR`, `WARN`, `FATAL` |
...
|---|---|---|---|---|---|---|
| [`SUSPEND_TASK_AFTER_NUM_FAILURES`](Parameters  Variables  Configuration/SUSPEND_TASK_AFTER_NUM_FAILURES.md) | Task property | Auto-suspend threshold | Integer >= 0 | `10` | `CREATE/ALTER TASK` | Halts task after N consecutive failures |
| [`ERROR_INTEGRATION`](Parameters  Variables  Configuration/ERROR_INTEGRATION.md) | Task property | Notification target | Notification integration name | None | `CREATE/ALTER TASK` | Routes failure alerts |
| [`USER_TASK_TIMEOUT_MS`](Parameters  Variables  Configuration/USER_TASK_TIMEOUT_MS.md) | Account parameter | Max task runtime | Milliseconds | `3600000` (1 hour) | Account | Cancels long-running tasks |
| [`ON_ERROR`](Parameters  Variables  Configuration/ON_ERROR.md) | COPY option | File load failure handling | `CONTINUE`, `SKIP_FILE`, `SKIP_FILE_<n>`, `ABORT_STATEMENT` | `ABORT_STATEMENT` | `COPY INTO`, Pipe | Determines whether bad files halt load |
| [`VALIDATION_MODE`](Parameters  Variables  Configuration/VALIDATION_MODE.md) | COPY option | Pre-validate without loading | `RETURN_N_ROWS`, `RETURN_ALL_ERRORS` | None | `COPY INTO` | Catches errors before persistence |
| [`FORCE`](Parameters  Variables  Configuration/FORCE.md) | COPY option | Override duplicate detection | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | `TRUE` risks duplicates; avoid in production |
| [`PIPE_EXECUTION_PAUSED`](Parameters  Variables  Configuration/PIPE_EXECUTION_PAUSED.md) | Pipe property | Pause pipe loading | `TRUE`, `FALSE` | `FALSE` | `ALTER PIPE` | Stops new file processing |
| [`QUERY_TAG`](Parameters  Variables  Configuration/QUERY_TAG.md) | Session parameter | Traceability | String | None | Task SQL | Enables failure correlation in history |
...
|---|---|---|
| [View task history](Privilege Requirements for Recovery/View task history.md) | `MONITOR` or `OWNERSHIP` | Task |
| [Resume suspended task](Privilege Requirements for Recovery/Resume suspended task.md) | `OPERATE` | Task |
| [Modify task SQL](Privilege Requirements for Recovery/Modify task SQL.md) | `OWNERSHIP` | Task |
| [Read quarantine tables](Privilege Requirements for Recovery/Read quarantine tables.md) | `SELECT` | Quarantine schema |
| [Purge quarantine data](Privilege Requirements for Recovery/Purge quarantine data.md) | `DELETE` | Quarantine table |
| [View pipe history](Privilege Requirements for Recovery/View pipe history.md) | `MONITOR` or `ACCOUNTADMIN` | Account |
