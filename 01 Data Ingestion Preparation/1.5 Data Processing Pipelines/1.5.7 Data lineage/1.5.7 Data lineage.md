...
|---|---|---|---|---|---|
| OBJECT_DEPENDENCIES | System view | Object-level dependency graph | DDL creating dependencies (views, UDFs, masking policies) | Referenced and referencing objects | Query-time |
| ACCESS_HISTORY | Account view | Query-level column/table access | Query execution | JSON arrays of accessed columns, tables, modified objects | Query-time, 45-min latency |
| SHOW OBJECTS | SQL command | Object enumeration | Database/schema | Object list with type, owner | DDL command |
| SHOW VIEWS | SQL command | View lineage inspection | Database/schema | View definitions, source tables | DDL command |
| GET_DDL | SQL function | Object definition extraction | Object name | CREATE statement text | Function call |
| TASK_HISTORY | Account view | Task execution lineage | Task scheduler | Predecessor/successor states, query IDs | Query-time, 45-min latency |
| SHOW TASKS | SQL command | Task graph enumeration | Database/schema | Task list with AFTER clauses | DDL command |
| SHOW PIPES | SQL command | Pipe source-to-target mapping | Database/schema | Pipe definition, source stage, target table | DDL command |
| SHOW STREAMS | SQL command | Stream source mapping | Database/schema | Stream name, source table, staleness | DDL command |
| TABLE_STORAGE_METRICS | System view | Storage-level object metadata | Table objects | Bytes, rows, clustering depth | Query-time |
| DATA_METRIC_FUNCTION_REFERENCES | System view | Quality rule bindings | Table-column to function mappings | Bound metrics and schedules | Query-time |
...
|---|---|---|---|
| `REFERENCING_DATABASE` | Context | One per dependency | Database of object that references |
| `REFERENCING_SCHEMA` | Context | One per dependency | Schema of referencing object |
| `REFERENCING_OBJECT_NAME` | Source | One per dependency | Object that has the dependency |
| `REFERENCING_OBJECT_DOMAIN` | Type | One per dependency | `VIEW`, `FUNCTION`, `MASKING POLICY`, etc. |
| `REFERENCED_DATABASE` | Context | One per dependency | Database of referenced object |
| `REFERENCED_SCHEMA` | Context | One per dependency | Schema of referenced object |
| `REFERENCED_OBJECT_NAME` | Target | One per dependency | Object being referenced |
| `REFERENCED_OBJECT_DOMAIN` | Type | One per dependency | `TABLE`, `VIEW`, etc. |
| `REFERENCED_OBJECT_ID` | Identifier | One per dependency | Internal ID |
...
| `DIRECT_OBJECTS_ACCESSED` | Immediate references | One per query | JSON array |
| `BASE_OBJECTS_ACCESSED` | Resolved references | One per query | JSON array (through views) |
| `OBJECTS_MODIFIED` | DML targets | One per query | JSON array |
| `COLUMNS_ACCESSED` | Column detail | One per query | JSON array |
...
| `DEFINITION` | SQL body | May reference tables directly |
| `WAREHOUSE` | Compute | |
| `SCHEDULE` | Trigger | |
...
| `DEFINITION` | SQL body | Contains `COPY INTO` with source and target |
| `NOTIFICATION_CHANNEL` | Event source | Cloud notification ARN/topic |
...
| `TABLE_NAME` | Source | Base table or view |
| `TABLE_DATABASE` | Source context | |
| `TABLE_SCHEMA` | Source context | |
| `STALE` | Health | `TRUE` if source DDL changed |
...
|---|---|---|---|---|---|---|
| `QUERY_TAG` | Session parameter | Lineage traceability | String <= 256 chars | None | Session/query | Annotates query history for pipeline correlation |
| `TIMEZONE` | Session parameter | Timestamp context | IANA timezone | `UTC` | Session | Affects lineage record timestamps |
| `DATA_RETENTION_TIME_IN_DAYS` | Object parameter | Historical lineage availability | 0-90 (Enterprise+) | `1` | Table/schema/database | Determines time travel for historical object states |
| `ENABLE_QUERY_ACCELERATION` | Warehouse parameter | Query performance | `TRUE`, `FALSE` | `FALSE` | Warehouse | May affect query plan resolution in access history |
...
|---|---|---|
| Query OBJECT_DEPENDENCIES | `USAGE` on database/schema | Database/Schema |
| Query ACCESS_HISTORY | `MONITOR` or `ACCOUNTADMIN` | Account |
| View task definitions | `MONITOR` or `OWNERSHIP` | Task |
| View pipe definitions | `USAGE` on schema | Schema |
| View stream definitions | `USAGE` on schema | Schema |
| Execute GET_DDL | `USAGE` + `SELECT` or `OWNERSHIP` | Target object |
