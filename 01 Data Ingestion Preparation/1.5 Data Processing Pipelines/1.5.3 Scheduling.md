...
|---|---|---|---|---|---|
| Task | Schema object | Executes SQL or procedure on schedule or trigger | SQL body, stored procedure call, predecessor state | DML/DDL execution, task history entry | Cron schedule, interval, or `AFTER` predecessor |
| Root Task | Task subtype | Initiates task graph execution | Cron expression or time interval | Triggers child tasks upon completion | Time-based scheduler |
| Child Task | Task subtype | Executes after predecessor tasks | Predecessor task success state | Runs when all `AFTER` tasks succeed | Dependency-based trigger |
| Serverless Task | Task variant | Uses Snowflake-managed compute | Same as standard task | Same as standard task | No warehouse specified; Snowflake provisions compute |
| Task Graph / DAG | Dependency structure | Defines execution order across tasks | Root task + child tasks with `AFTER` | Serialized or parallel stage execution | Implicit graph traversal |
| Cron Schedule | Schedule expression | Defines calendar-based execution times | Cron string + optional timezone | Task scheduled at specified times | Parsed by task scheduler |
| Interval Schedule | Schedule expression | Defines fixed-frequency execution | `N MINUTES` | Task scheduled every N minutes | Parsed by task scheduler |
| WHEN Clause | Conditional filter | Skips execution if condition is false | Boolean expression | Task state `SKIPPED` or `RUNNING` | Evaluated at schedule time |
| ERROR_INTEGRATION | Notification binding | Alerts on task state changes | Task completion/failure | Notification payload to external system | Automatic on state transition |
...
|---|---|---|---|
| `NAME` | Task identifier | One per execution | Schema-qualified task name |
| `QUERY_ID` | Execution trace | One per run | Joins to `QUERY_HISTORY` |
| `DATABASE_NAME` | Context | One per task | |
| `SCHEMA_NAME` | Context | One per task | |
| `SCHEDULED_TIME` | Intended start | One per run | Cron-derived or predecessor-derived |
| `COMPLETED_TIME` | Actual end | One per run | |
| `STATE` | Outcome | One per run | `SCHEDULED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `SKIPPED` |
| `ERROR_CODE` | Failure code | One per failed run | Null on success |
| `ERROR_MESSAGE` | Failure detail | One per failed run | Null on success |
| `RUN_ID` | Instance ID | One per run | Monotonically increasing |
| `CONFIG` | Task definition | One per run | JSON of warehouse, schedule, etc. |
| `GRAPH_VERSION` | DAG version | One per run | Incremented on graph changes |
...
| `TASK_SCHEMA` | Context | |
| `TASK_DATABASE` | Context | |
| `SCHEDULE` | Trigger definition | Cron or interval string |
| `WAREHOUSE` | Compute target | Null for serverless |
| `PREDECESSORS` | Graph edges | Comma-separated task names |
| `STATE` | Current state | `SUSPENDED` or `STARTED` |
| `DEFINITION` | SQL body | Task execution statement |
| `CONDITION_TEXT` | WHEN clause | Null if unconditional |
| `ERROR_INTEGRATION` | Notification target | Null if unconfigured |
...
| `QUERY_ID` | Execution trace | |
| `DATABASE_NAME` | Context | |
| `SCHEMA_NAME` | Context | |
| `SCHEDULED_TIME` | Intended start | |
| `COMPLETED_TIME` | Actual end | |
| `STATE` | Outcome | |
| `ERROR_CODE` | Failure code | |
| `ERROR_MESSAGE` | Failure detail | |
| `RUN_ID` | Instance ID | |
| `GRAPH_VERSION` | DAG version | |
...
|---|---|---|---|---|---|---|
| `SCHEDULE` | Task property | Time-based trigger | `USING CRON '...' [TIMEZONE '...']` or `N MINUTES` | None (required for root) | `CREATE/ALTER TASK` | Defines when root task executes |
| `AFTER` | Task property | Dependency trigger | Comma-separated task names | None | `CREATE/ALTER TASK` | Defines child task predecessors |
| `WHEN` | Task property | Conditional guard | Boolean SQL expression | None (always run) | `CREATE/ALTER TASK` | Skips execution if false |
| `WAREHOUSE` | Task property | Compute target | Warehouse name or omitted | Omitted (serverless) | `CREATE/ALTER TASK` | Determines provisioned vs serverless |
| `ERROR_INTEGRATION` | Task property | Alert routing | Notification integration name | None | `CREATE/ALTER TASK` | Sends failure/success notifications |
| `USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE` | Task property | Serverless hint | `XSMALL`, `SMALL`, `MEDIUM`, `LARGE`, `XLARGE`, `XXLARGE` | `XSMALL` | `CREATE/ALTER TASK` | Initial compute allocation for serverless |
| `USER_TASK_TIMEOUT_MS` | Account parameter | Max runtime | Integer milliseconds | `3600000` (1 hour) | Account | Aborts tasks exceeding limit |
| `SUSPEND_TASK_AFTER_NUM_FAILURES` | Task property | Auto-suspend threshold | Integer >= 0 | `10` | `CREATE/ALTER TASK` | Suspends task after N consecutive failures |
| `TIMEZONE` | Schedule modifier | Cron timezone | IANA timezone string | `UTC` | `SCHEDULE` clause | Shifts cron fire times from UTC |
| `GRAPH_VERSION` | System metadata | DAG versioning | Integer | Auto-incremented | System | Tracks graph structural changes |
| `QUERY_TAG` | Session parameter | Traceability | String | None | Task SQL body | Enables cost and performance tracking |
...
| Resume/suspend own task | `OPERATE` | Task |
| Resume/suspend any task | `EXECUTE TASK` | Account |
| Modify task definition | `OWNERSHIP` | Task |
| Use warehouse | `USAGE` | Warehouse |
| Read source data | `SELECT` | Table/Stream/View |
| Write target data | `INSERT`, `UPDATE`, `DELETE` | Target table |
| View task history | `MONITOR` or `OWNERSHIP` | Task or account |
