...
|---|---|---|---|---|---|
| [Staging Table](SQL Object Summary/Staging Table.md) | Table | Landing zone for raw data | `COPY INTO`, external tables, streams | Unvalidated rows in source format | Batch or continuous |
| [Stream (CDC)](SQL Object Summary/Stream (CDC).md) | Stream object | Captures change set on source table | DML on staging or source table | Delta rows with metadata columns | Automatic, transaction-bound |
| [Cleansing CTE/View](SQL Object Summary/Cleansing CTEView.md) | SQL pattern | Applies null handling, trimming, deduplication | Staging table or stream | Validated, standardized rows | Query-time or materialized |
| [Conforming CTE/View](SQL Object Summary/Conforming CTEView.md) | SQL pattern | Schema alignment, type coercion, grain fix | Cleansed output | Structurally consistent rows | Query-time or materialized |
| [Enrichment CTE/View](SQL Object Summary/Enrichment CTEView.md) | SQL pattern | Joins, lookups, derived columns | Conformed output plus reference tables | Augmented rows with new attributes | Query-time or materialized |
| [Target Table](SQL Object Summary/Target Table.md) | Table | Production-ready dataset | Enrichment output | Clean, conformed, enriched rows | `INSERT` / `MERGE` / `CREATE TABLE` |
| [Task](SQL Object Summary/Task.md) | Schema object | Schedules or chains pipeline execution | SQL statement or stored procedure | Executed DML at interval or trigger | Time-based or predecessor-based |
| [UDF / External Function](SQL Object Summary/UDF  External Function.md) | Function | Encapsulates complex transformation logic | Input row or column value | Transformed or looked-up value | Per-row invocation during query |
| [MERGE Statement](SQL Object Summary/MERGE Statement.md) | DML pattern | Incremental upsert into target | Source stream/CTEs plus target table | Inserted, updated, or unchanged rows | Transactional |
...
|---|---|---|---|
| [`RAW_DATA`](Staging Table (Example Structure)/RAW_DATA.md) | VARIANT | One per source record | Nullable; holds entire payload |
| [`LOAD_TIMESTAMP`](Target Production Table/LOAD_TIMESTAMP.md) | TIMESTAMP_NTZ | One per load batch | `DEFAULT CURRENT_TIMESTAMP()` |
| [`FILE_NAME`](Staging Table (Example Structure)/FILE_NAME.md) | VARCHAR | One per source file | From `METADATA$FILENAME` |
| [`ROW_NUMBER`](Staging Table (Example Structure)/ROW_NUMBER.md) | NUMBER | One per record | Computed during load |

## Stream on Staging Table

| Column | Role | Notes |
|---|---|---|
| [`METADATA$ACTION`](Stream on Staging Table/METADATA$ACTION.md) | Delta type | `INSERT` or `DELETE` |
| [`METADATA$ISUPDATE`](Stream on Staging Table/METADATA$ISUPDATE.md) | Update flag | `TRUE` if update presented as delete+insert |
| [`METADATA$ROW_ID`](Stream on Staging Table/METADATA$ROW_ID.md) | Unique delta ID | Identifies specific change record |

## Target Production Table

| Column | Role | Grain | Source |
|---|---|---|---|
| [`SURROGATE_KEY`](Target Production Table/SURROGATE_KEY.md) | Primary identifier | One per business entity | Hash or sequence |
| [`BUSINESS_KEY`](Target Production Table/BUSINESS_KEY.md) | Natural identifier | One per business entity | Source system |
| [`CLEANSED_ATTRIBUTE`](Target Production Table/CLEANSED_ATTRIBUTE.md) | Cleaned value | One per entity attribute | Cleansing CTE |
| [`CONFORMED_CODE`](Target Production Table/CONFORMED_CODE.md) | Standardized code | One per entity attribute | Conforming CTE + mapping table |
| [`ENRICHED_ATTRIBUTE`](Target Production Table/ENRICHED_ATTRIBUTE.md) | Derived value | One per entity attribute | Enrichment CTE |
| [`VALID_FROM`](Target Production Table/VALID_FROM.md) | Temporal start | One per entity version | Enrichment CTE |
| [`VALID_TO`](Target Production Table/VALID_TO.md) | Temporal end | One per entity version | Enrichment CTE |
| [`LOAD_TIMESTAMP`](Target Production Table/LOAD_TIMESTAMP.md) | Audit timestamp | One per record | Pipeline execution time |
...
|---|---|---|---|---|---|---|
| [`ON_ERROR`](Parameters  Variables  Configuration/ON_ERROR.md) | COPY option | Controls load behavior on error | `CONTINUE`, `SKIP_FILE`, `ABORT_STATEMENT` | `ABORT_STATEMENT` | `COPY INTO` staging | Determines whether bad rows halt load or are quarantined |
| [`PURGE`](Parameters  Variables  Configuration/PURGE.md) | COPY option | Removes files after load | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | Prevents reprocessing if `TRUE` |
| [`VALIDATION_MODE`](Parameters  Variables  Configuration/VALIDATION_MODE.md) | COPY option | Pre-validate without loading | `RETURN_N_ROWS`, `RETURN_ALL_ERRORS` | None | `COPY INTO` | Catches errors before staging |
| [`WAREHOUSE`](Parameters  Variables  Configuration/WAREHOUSE.md) | Task property | Compute for task execution | Valid warehouse name | None | `CREATE TASK` | Determines performance and cost |
| [`SCHEDULE`](Parameters  Variables  Configuration/SCHEDULE.md) | Task property | Execution cadence | `USING CRON ...` or `N MINUTES` | None | `CREATE TASK` | Defines pipeline frequency |
| [`ERROR_INTEGRATION`](Parameters  Variables  Configuration/ERROR_INTEGRATION.md) | Task property | Notification target | Notification integration name | None | `CREATE TASK` | Alerts on task failure |
| [`TIMEZONE`](Parameters  Variables  Configuration/TIMEZONE.md) | Session parameter | Timestamp context | IANA timezone | `UTC` | Session | Affects `CURRENT_TIMESTAMP` and date functions |
...
| [`QUERY_TAG`](Parameters  Variables  Configuration/QUERY_TAG.md) | Session parameter | Pipeline identification | String | None | Session | Enables workload monitoring and cost attribution |
