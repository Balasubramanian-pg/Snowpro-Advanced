...
|---|---|---|---|---|---|
| GEOGRAPHY | Data type | Ellipsoidal coordinate storage | Latitude/longitude or WKT/GeoJSON | Binary-encoded geospatial object | Column definition, CAST, constructor |
| GEOMETRY | Data type | Planar coordinate storage | X/Y coordinates or WKT/GeoJSON | Binary-encoded geospatial object | Column definition, CAST, constructor |
| ST_MAKEPOINT | Constructor | Create point from coordinates | Longitude, latitude (GEOGRAPHY) or X, Y (GEOMETRY) | Point geometry | Scalar function |
| ST_MAKELINE | Constructor | Create line from points | Array of points or point columns | Line geometry | Aggregate or scalar function |
| ST_MAKEPOLYGON | Constructor | Create polygon from ring | Line ring or array of points | Polygon geometry | Scalar function |
| ST_DISTANCE | Measurement | Distance between two geometries | Two geospatial objects | Distance in meters (GEOGRAPHY) or units (GEOMETRY) | Scalar function |
| ST_DWITHIN | Predicate | Within-distance test | Two geospatial objects, distance threshold | Boolean | Scalar function |
| ST_LENGTH | Measurement | Length of line geometry | Line geometry | Length in meters (GEOGRAPHY) | Scalar function |
| ST_AREA | Measurement | Area of polygon | Polygon geometry | Area in square meters (GEOGRAPHY) | Scalar function |
| ST_INTERSECTS | Predicate | Intersection test | Two geospatial objects | Boolean | Scalar function |
| ST_CONTAINS | Predicate | Containment test | Outer geometry, inner geometry | Boolean | Scalar function |
| ST_WITHIN | Predicate | Within test | Inner geometry, outer geometry | Boolean | Scalar function |
| ST_CENTROID | Transformation | Compute centroid | Polygon or multipolygon | Point geometry | Scalar function |
| ST_BUFFER | Transformation | Expand geometry by radius | Geometry, radius distance | Expanded polygon | Scalar function |
| ST_COLLECT | Aggregate | Collect geometries into multi-geometry | Set of geometries | Multi-point/line/polygon | Aggregate function |
| ST_ASGEOJSON | Conversion | Export to GeoJSON | GEOGRAPHY/GEOMETRY | GeoJSON string | Scalar function |
| ST_ASWKT | Conversion | Export to WKT | GEOGRAPHY/GEOMETRY | WKT string | Scalar function |
| ST_GEOGFROMWKT | Conversion | Parse WKT to GEOGRAPHY | WKT string | GEOGRAPHY object | Scalar function |
| ST_GEOGFROMGEOJSON | Conversion | Parse GeoJSON to GEOGRAPHY | GeoJSON string | GEOGRAPHY object | Scalar function |
| ST_X / ST_Y | Accessor | Extract coordinates | Point geometry | Coordinate number | Scalar function |
| ST_SIMPLIFY | Transformation | Reduce vertex count | Geometry, tolerance | Simplified geometry | Scalar function |
| ST_TRANSFORM | Transformation | Reproject coordinates | Geometry, source SRID, target SRID | Reprojected geometry | Scalar function |
...
|---|---|---|
| Internal format | Binary | Compressed WKB-like encoding |
| Coordinate system | WGS 84 | EPSG:4326 |
| Units | Degrees input, meters output | Distance/area return meters/sq meters |
| Precision | Double-precision floating point | ~15 decimal digits |
| Validity | Enforced at construction | Invalid coordinates rejected |

## GEOMETRY Column Storage

| Attribute | Role | Notes |
|---|---|---|
| Internal format | Binary | Compressed planar encoding |
| Coordinate system | Cartesian | Defined by SRID |
| Units | Coordinate units | Input and output in same units |
| SRID | Spatial reference | Default 0; use ST_TRANSFORM to reproject |

## Point Table (Example)

| Column | Role | Type | Grain | Notes |
|---|---|---|---|---|
| `LOCATION_ID` | Identifier | VARCHAR | One per location | Business key |
| `GEO` | Geospatial | GEOGRAPHY | One per location | `ST_MAKEPOINT(lng, lat)` |
| `LATITUDE` | Raw coordinate | NUMBER | One per location | Source latitude |
| `LONGITUDE` | Raw coordinate | NUMBER | One per location | Source longitude |
| `CATEGORY` | Dimension | VARCHAR | One per location | Classification |
...
| Column | Role | Type | Grain | Notes |
|---|---|---|---|---|
| `REGION_ID` | Identifier | VARCHAR | One per region | Business key |
| `BOUNDARY` | Geospatial | GEOGRAPHY | One per region | `ST_MAKEPOLYGON(ring)` |
| `AREA_SQ_M` | Cached metric | NUMBER | One per region | `ST_AREA(BOUNDARY)` |
| `CENTER` | Cached centroid | GEOGRAPHY | One per region | `ST_CENTROID(BOUNDARY)` |
...
|---|---|---|---|---|---|---|
| `GEOGRAPHY` | Data type | Ellipsoidal storage | Binary | N/A | Column definition | WGS 84 geodetic geometry |
| `GEOMETRY` | Data type | Planar storage | Binary | N/A | Column definition | Cartesian geometry |
| `ST_MAKEPOINT` | Function args | Point construction | Two numbers | Required | Constructor | Longitude/X first, latitude/Y second |
| `ST_BUFFER` radius | Function arg | Buffer distance | Number >= 0 | Required | Transformation | Meters for GEOGRAPHY |
| `ST_DWITHIN` distance | Function arg | Distance threshold | Number >= 0 | Required | Predicate | Meters for GEOGRAPHY |
| `ST_SIMPLIFY` tolerance | Function arg | Simplification threshold | Number >= 0 | Required | Transformation | Meters for GEOGRAPHY |
| `ST_TRANSFORM` SRID | Function args | Reprojection | Source SRID, target SRID | Required | Transformation | Coordinate system conversion |
| `ST_GEOGFROMGEOJSON` | Function arg | Parse GeoJSON | Valid GeoJSON string | Required | Conversion | Returns GEOGRAPHY |
| `ST_GEOGFROMWKT` | Function arg | Parse WKT | Valid WKT string | Required | Conversion | Returns GEOGRAPHY |
| `ST_ASGEOJSON` | Function arg | Export GeoJSON | GEOGRAPHY/GEOMETRY | Required | Conversion | Returns GeoJSON string |
| `ST_ASWKT` | Function arg | Export WKT | GEOGRAPHY/GEOMETRY | Required | Conversion | Returns WKT string |
| `TIMEZONE` | Session parameter | Temporal context | IANA timezone | `UTC` | Session | Affects timestamp context |
...
|---|---|---|
| Create tables with geospatial columns | `CREATE TABLE` on schema | Schema |
| Insert geospatial data | `INSERT` on table | Table |
| Query geospatial columns | `SELECT` on table | Table |
| Use geospatial functions | None (built-in) | N/A |
| Create UDFs with geospatial logic | `CREATE FUNCTION` on schema | Schema |
