...
|---|---|---|---|---|---|
| FLATTEN | Built-in table function | Unnest VARIANT arrays/objects into rows | VARIANT, ARRAY, or OBJECT expression | Relational rows with path, index, key, value, type | `FROM` clause with `LATERAL` |
| SPLIT_TO_TABLE | Built-in table function | Tokenize string into rows | String expression, delimiter | Rows with index and token value | `FROM` clause with `LATERAL` |
| GENERATOR | Built-in table function | Generate synthetic rows | Row count or time range | Specified number of rows | `FROM` clause |
| RESULT_SCAN | Built-in table function | Access query result set as table | Query ID | Rows from historical query result | `FROM` clause |
| TABLE / INFORMATION_SCHEMA functions | System table functions | List objects and metadata | Database/schema context | Object inventory rows | `FROM` clause |
| EXTERNAL_TABLE_FILES | System table function | List files referenced by external table | External table name | File metadata rows | `FROM` clause |
| EXTERNAL_TABLE_PARTITIONS | System table function | List partitions of external table | External table name | Partition metadata rows | `FROM` clause |
| SQL UDTF | User-defined table function | Custom row-set logic in SQL | Input arguments | Result set rows | `CREATE FUNCTION` with `RETURNS TABLE` |
| JavaScript/Python/Java UDTF | User-defined table function | Custom row-set logic in guest runtime | Input arguments | Result set rows | `CREATE FUNCTION` with `RETURNS TABLE` |
| LATERAL | Join modifier | Correlate table function to preceding row | Table function expression | Correlated row expansion | `FROM` clause |
...
|---|---|---|---|
| `SEQ` | Sequence | NUMBER | Unique within flatten invocation |
...
| `VALUE` | Element value | VARIANT | The flattened value |
...
| Column | Role | Type | Notes |
|---|---|---|---|
| `INDEX` | Position | NUMBER | 1-based token index |
| `VALUE` | Token | VARCHAR | Substring between delimiters |
...
| Column | Role | Type | Notes |
|---|---|---|---|
| `SEQ` / `TIMESTAMP` | Generated value | NUMBER / TIMESTAMP | Sequence number or time step |
...
| Column | Role | Type | Notes |
|---|---|---|---|
| User-defined columns | Output data | As declared | Defined in `RETURNS TABLE (...)` |
...
|---|---|---|---|---|---|---|
| `input` | FLATTEN arg | Source data | VARIANT/ARRAY/OBJECT expression | Required | `FLATTEN` | Data to flatten |
| `path` | FLATTEN arg | JSON path | String path | None (root) | `FLATTEN` | Nested structure target |
| `outer` | FLATTEN arg | Empty handling | `TRUE`, `FALSE` | `FALSE` | `FLATTEN` | Returns null row for empty input |
| `recursive` | FLATTEN arg | Deep flatten | `TRUE`, `FALSE` | `FALSE` | `FLATTEN` | Recursively flattens nested structures |
| `mode` | FLATTEN arg | Input type hint | `'OBJECT'`, `'ARRAY'`, `'BOTH'` | `'BOTH'` | `FLATTEN` | Treats input as object or array |
| `ROWCOUNT` | GENERATOR arg | Row count | Integer >= 0 | None | `GENERATOR` | Number of rows to generate |
| `TIMESTEP` | GENERATOR arg | Time interval | Interval string | None | `GENERATOR` | Time increment for series |
| `START` | GENERATOR arg | Series start | Timestamp | None | `GENERATOR` | Series start time |
| `STOP` | GENERATOR arg | Series end | Timestamp | None | `GENERATOR` | Series end time |
| `query_id` | RESULT_SCAN arg | Source query | Valid query ID | Required | `RESULT_SCAN` | Query to introspect |
| `LANGUAGE` | UDTF property | Runtime | `SQL`, `JAVASCRIPT`, `PYTHON`, `JAVA`, `SCALA` | `SQL` | `CREATE FUNCTION` | Execution environment |
| `RETURNS TABLE` | UDTF property | Output schema | Column list with types | Required | `CREATE FUNCTION` | Defines result structure |
| `IS_SECURE` | UDTF property | Visibility | `TRUE`, `FALSE` | `FALSE` | `CREATE FUNCTION` | Obscures definition |
...
|---|---|---|
| Use FLATTEN/SPLIT_TO_TABLE | `SELECT` on source table | Table |
| Use GENERATOR | None | N/A |
| Use RESULT_SCAN | `SELECT` on result (implicit) | Query result |
| Create UDTF | `CREATE FUNCTION` on schema | Schema |
| Call UDTF | `USAGE` on function | Function |
| Use system table functions | `USAGE` on database/schema | Context objects |
