...
|---|---|---|---|---|---|
| ROW_NUMBER | Window function | Assign unique sequential integer per partition | `PARTITION BY`, `ORDER BY` | Integer 1..N per partition | Per-row during query execution |
| RANK | Window function | Assign rank with gaps for ties | `PARTITION BY`, `ORDER BY` | Integer rank; ties share rank, next rank skips | Per-row during query execution |
| DENSE_RANK | Window function | Assign rank without gaps for ties | `PARTITION BY`, `ORDER BY` | Integer rank; ties share rank, next rank consecutive | Per-row during query execution |
| NTILE | Window function | Distribute rows into N buckets | `PARTITION BY`, `ORDER BY`, bucket count | Integer bucket number 1..N | Per-row during query execution |
| LAG | Window function | Access preceding row value | `PARTITION BY`, `ORDER BY`, offset | Value from N rows before current | Per-row during query execution |
| LEAD | Window function | Access following row value | `PARTITION BY`, `ORDER BY`, offset | Value from N rows after current | Per-row during query execution |
| FIRST_VALUE | Window function | First value in window frame | `PARTITION BY`, `ORDER BY`, frame | First ordered value | Per-row during query execution |
| LAST_VALUE | Window function | Last value in window frame | `PARTITION BY`, `ORDER BY`, frame | Last ordered value | Per-row during query execution |
| NTH_VALUE | Window function | Nth value in window frame | `PARTITION BY`, `ORDER BY`, frame, N | Nth ordered value | Per-row during query execution |
| SUM/AVG/COUNT (window) | Aggregate window function | Running/cumulative aggregates | `PARTITION BY`, `ORDER BY`, frame | Cumulative or sliding aggregate | Per-row during query execution |
| MIN/MAX (window) | Aggregate window function | Running extremum | `PARTITION BY`, `ORDER BY`, frame | Sliding minimum/maximum | Per-row during query execution |
| PERCENT_RANK | Window function | Relative rank within partition | `PARTITION BY`, `ORDER BY` | Float 0..1 | Per-row during query execution |
| CUME_DIST | Window function | Cumulative distribution | `PARTITION BY`, `ORDER BY` | Float 0..1 | Per-row during query execution |
| PERCENTILE_CONT | Window function | Continuous percentile | `WITHIN GROUP (ORDER BY)`, percentile | Interpolated percentile value | Per-row during query execution |
| PERCENTILE_DISC | Window function | Discrete percentile | `WITHIN GROUP (ORDER BY)`, percentile | Exact percentile value | Per-row during query execution |
| RATIO_TO_REPORT | Window function | Proportion of partition total | `PARTITION BY`, value expression | Float proportion | Per-row during query execution |
| LISTAGG (window) | Window function | Ordered string collection per frame | `PARTITION BY`, `ORDER BY`, frame | Concatenated string | Per-row during query execution |
| ROWS | Frame unit | Row-based frame bounds | `UNBOUNDED PRECEDING`, `N PRECEDING`, `CURRENT ROW`, `N FOLLOWING`, `UNBOUNDED FOLLOWING` | Frame boundary | Window specification |
| RANGE | Frame unit | Value-based frame bounds | Same keywords as ROWS | Frame boundary based on ORDER BY value differences | Window specification |
| GROUPS | Frame unit | Peer-group-based frame bounds | Same keywords as ROWS | Frame boundary based on peer groups | Window specification |
| NULLS FIRST / NULLS LAST | Ordering modifier | Null placement in ordering | `NULLS FIRST`, `NULLS LAST` | Null ordering position | `ORDER BY` clause |
...
|---|---|---|---|
| Original columns | Source data | One per row | From input row set |
| `ROW_NUMBER()` | Ranking | One per row | Unique within partition |
| `RANK()` | Ranking | One per row | With gaps for ties |
| `DENSE_RANK()` | Ranking | One per row | Without gaps for ties |
| `LAG(col, offset)` | Offset | One per row | Value from preceding row |
| `LEAD(col, offset)` | Offset | One per row | Value from following row |
| `SUM(col) OVER (...)` | Aggregate window | One per row | Cumulative or sliding sum |
| `PERCENT_RANK()` | Distribution | One per row | Relative position 0..1 |
...
|---|---|---|---|
| `ROWS` | Physical row offset | Running totals, fixed lookback | Fast; simple arithmetic |
| `RANGE` | Value difference from ORDER BY | Value-based windows (e.g., 7 days) | Slower; requires value comparison |
| `GROUPS` | Peer group offset | Tie-aware windows | Moderate; peer group tracking |

## Default Frame Behavior

| Function Category | Default Frame | Notes |
|---|---|---|
| Ranking (`ROW_NUMBER`, `RANK`, etc.) | `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` | Frame irrelevant for ranking |
| Offset (`LAG`, `LEAD`) | Row-based implicit | Offset is physical rows |
| Aggregate window (`SUM`, `AVG`) | `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` | Cumulative from partition start |
| `FIRST_VALUE`, `NTH_VALUE` | `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` | Must override for full partition |
| `LAST_VALUE` | `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` | **Always returns current row unless frame overridden** |
...
|---|---|---|---|---|---|---|
| `PARTITION BY` | Window clause | Row grouping | Expression list | None (single partition) | `OVER()` | Divides rows into independent groups |
| `ORDER BY` | Window clause | Row ordering | Expression list | None | `OVER()` | Defines row sequence within partition |
| `ROWS` | Frame unit | Physical bounds | `UNBOUNDED PRECEDING`, `N PRECEDING`, `CURRENT ROW`, `N FOLLOWING`, `UNBOUNDED FOLLOWING` | `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` | `OVER()` | Frame based on row position |
| `RANGE` | Frame unit | Value bounds | Same as ROWS | Same default | `OVER()` | Frame based on ORDER BY value difference |
| `GROUPS` | Frame unit | Peer bounds | Same as ROWS | Same default | `OVER()` | Frame based on peer group position |
| `NULLS FIRST` | Ordering modifier | Null placement | `NULLS FIRST`, `NULLS LAST` | `NULLS LAST` (ASC), `NULLS FIRST` (DESC) | `ORDER BY` | Controls null ordering |
| `LAG/LEAD offset` | Function arg | Row offset | Integer >= 0 | `1` | `LAG`/`LEAD` | Number of rows to look back/forward |
| `LAG/LEAD default` | Function arg | OOB value | Any compatible value | `NULL` | `LAG`/`LEAD` | Value when offset beyond boundary |
| `NTILE buckets` | Function arg | Bucket count | Integer > 0 | Required | `NTILE` | Number of groups to create |
| `NTH_VALUE N` | Function arg | Position | Integer > 0 | Required | `NTH_VALUE` | Which value to return |
| `TIMEZONE` | Session parameter | Temporal context | IANA timezone | `UTC` | Session | Affects date/time ordering |
...
|---|---|---|
| Query window functions | `SELECT` on table | Table |
| Use window UDFs | `USAGE` on function | Function |
