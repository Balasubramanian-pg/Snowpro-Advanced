...
|---|---|---|---|---|---|
| SQL Scalar UDF | UDF | Custom single-value logic in SQL | Input arguments | Single value per row | `CREATE FUNCTION` with SQL expression |
| SQL Table UDF | UDTF | Custom row-set logic in SQL | Input arguments | Result set | `CREATE FUNCTION ... RETURNS TABLE` with SELECT |
| JavaScript Scalar UDF | UDF | Custom logic in JavaScript | Input arguments | Single value per row | `CREATE FUNCTION ... LANGUAGE JAVASCRIPT` |
| JavaScript Table UDF | UDTF | Custom row-set logic in JavaScript | Input arguments | Result set | `CREATE FUNCTION ... RETURNS TABLE ... LANGUAGE JAVASCRIPT` |
| Python Scalar UDF | UDF | Custom logic in Python | Input arguments | Single value per row | `CREATE FUNCTION ... LANGUAGE PYTHON` |
| Python Table UDF | UDTF | Custom row-set logic in Python | Input arguments | Result set | `CREATE FUNCTION ... RETURNS TABLE ... LANGUAGE PYTHON` |
| Java Scalar UDF | UDF | Custom logic in Java | Input arguments | Single value per row | `CREATE FUNCTION ... LANGUAGE JAVA` |
| Java Table UDF | UDTF | Custom row-set logic in Java | Input arguments | Result set | `CREATE FUNCTION ... RETURNS TABLE ... LANGUAGE JAVA` |
| Secure UDF | UDF modifier | Obscures definition | Same as base UDF | Same as base UDF | `CREATE SECURE FUNCTION` |
| Immutable UDF | UDF property | Deterministic result | Input values only | Consistent output for same input | `IMMUTABLE` keyword |
| Volatile UDF | UDF property | Non-deterministic result | Input values + system state | Variable output | `VOLATILE` keyword |
| Overloaded UDF | UDF variant | Multiple signatures | Distinct argument types | Type-specific behavior | Multiple `CREATE FUNCTION` with same name |
...
| Column | Role | Notes |
|---|---|---|
| `FUNCTION_NAME` | Identifier | May be overloaded |
| `FUNCTION_SCHEMA` | Context | |
| `FUNCTION_CATALOG` | Context | |
| `FUNCTION_OWNER` | Privilege context | Determines execution privileges |
| `ARGUMENT_SIGNATURE` | Overload key | JSON array of argument types |
| `DATA_TYPE` | Return type | Scalar type or `TABLE` |
| `FUNCTION_LANGUAGE` | Runtime | `SQL`, `JAVASCRIPT`, `PYTHON`, `JAVA`, `SCALA` |
| `IS_SECURE` | Visibility | `YES` or `NO` |
| `IS_VOLATILE` | Determinism | `YES` (volatile) or `NO` (immutable) |

## UDTF Return Schema (Declared)

| Column | Role | Notes |
|---|---|---|
| `COL1` | Output column | As declared in `RETURNS TABLE (col1 TYPE1, ...)` |
| `COL2` | Output column | |
| ... | | |
...
|---|---|---|---|---|---|---|
| `LANGUAGE` | UDF property | Runtime | `SQL`, `JAVASCRIPT`, `PYTHON`, `JAVA`, `SCALA` | `SQL` | `CREATE FUNCTION` | Execution environment |
| `RETURNS` | UDF property | Scalar return type | Any valid Snowflake type | Required | `CREATE FUNCTION` | Output type for scalar |
| `RETURNS TABLE` | UDF property | Table return schema | `(col1 TYPE1, ...)` | Required for UDTF | `CREATE FUNCTION` | Output schema for table |
| `IS_SECURE` | UDF property | Visibility | `TRUE`, `FALSE` | `FALSE` | `CREATE FUNCTION` | Obscures definition |
| `IMMUTABLE` | UDF property | Determinism | Implicit or explicit | `IMMUTABLE` | `CREATE FUNCTION` | Enables caching/inlining |
| `VOLATILE` | UDF property | Non-determinism | Explicit | None | `CREATE FUNCTION` | Disables caching |
| `RUNTIME_VERSION` | Python UDF | Python version | `3.8`, `3.9`, `3.10`, `3.11` | `3.8` | `CREATE FUNCTION` | Interpreter version |
| `HANDLER` | Python/Java UDF | Entry point | Function/class.method name | Required | `CREATE FUNCTION` | Callable entry |
| `PACKAGES` | Python UDF | Dependencies | List of package specs | None | `CREATE FUNCTION` | Third-party libraries |
| `IMPORTS` | Python/Java UDF | File dependencies | Stage file paths | None | `CREATE FUNCTION` | Additional files |
| `MAX_BATCH_ROWS` | Python UDF | Batch size | Integer | Varies | `CREATE FUNCTION` | Rows per batch |
| `TIMEZONE` | Session parameter | Temporal context | IANA timezone | `UTC` | Session | Affects time functions in UDF |
| `QUERY_TAG` | Session parameter | Traceability | String | None | Session | Tags UDF queries in history |
...
|---|---|---|
| Create UDF | `CREATE FUNCTION` on schema | Schema |
| Call UDF | `USAGE` on function | Function |
| View UDF definition | `OWNERSHIP` or `OPERATE` | Function |
| Call secure UDF | `USAGE` on function | Function |
| Reference objects in UDF | Privileges on referenced objects | Tables/Views/Stages |
