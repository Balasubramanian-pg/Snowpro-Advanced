...
|---|---|---|---|---|---|
| [SNOWFLAKE.CORTEX.COMPLETE](SQL Object Summary/SNOWFLAKE.CORTEX.COMPLETE.md) | LLM function | Text generation/completion | Prompt string, model name, optional parameters | Generated text string | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.COMPLETE_JSON](SQL Object Summary/SNOWFLAKE.CORTEX.COMPLETE_JSON.md) | LLM function | Structured JSON generation | Prompt string, model name | Valid JSON string | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.SUMMARIZE](SQL Object Summary/SNOWFLAKE.CORTEX.SUMMARIZE.md) | LLM function | Text summarization | Input text, model name | Summary string | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.TRANSLATE](SQL Object Summary/SNOWFLAKE.CORTEX.TRANSLATE.md) | LLM function | Text translation | Input text, source language, target language, model name | Translated text string | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.SENTIMENT](SQL Object Summary/SNOWFLAKE.CORTEX.SENTIMENT.md) | LLM function | Sentiment analysis | Input text | Sentiment score float (-1 to 1) | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.CLASSIFY_TEXT](SQL Object Summary/SNOWFLAKE.CORTEX.CLASSIFY_TEXT.md) | LLM function | Zero-shot classification | Input text, candidate labels | Label and score | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.EXTRACT_ANSWER](SQL Object Summary/SNOWFLAKE.CORTEX.EXTRACT_ANSWER.md) | LLM function | Extractive QA | Input text, question | Answer string or NULL | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.EMBED_TEXT_768](SQL Object Summary/SNOWFLAKE.CORTEX.EMBED_TEXT_768.md) | Embedding function | Text embedding (768-dim) | Input text, model name | VECTOR(FLOAT, 768) | Per-row SQL invocation |
| [SNOWFLAKE.CORTEX.EMBED_TEXT_1024](SQL Object Summary/SNOWFLAKE.CORTEX.EMBED_TEXT_1024.md) | Embedding function | Text embedding (1024-dim) | Input text, model name | VECTOR(FLOAT, 1024) | Per-row SQL invocation |
| [VECTOR_INNER_PRODUCT](SQL Object Summary/VECTOR_INNER_PRODUCT.md) | Vector function | Dot product similarity | Two VECTOR columns | Float scalar | Per-row SQL invocation |
| [VECTOR_L2_DISTANCE](SQL Object Summary/VECTOR_L2_DISTANCE.md) | Vector function | Euclidean distance | Two VECTOR columns | Float scalar | Per-row SQL invocation |
| [VECTOR_COSINE_DISTANCE](SQL Object Summary/VECTOR_COSINE_DISTANCE.md) | Vector function | Cosine distance | Two VECTOR columns | Float scalar (0 to 2) | Per-row SQL invocation |
| [VECTOR](SQL Object Summary/VECTOR.md) | Data type | Fixed-dimension float array | Dimension and element type | Binary-encoded vector | Column definition |
| [SNOWFLAKE.ML.FORECAST](SQL Object Summary/SNOWFLAKE.ML.FORECAST.md) | ML function | Time-series forecasting | Historical data, target column, timestamp | Forecast result set | Table function |
| [SNOWFLAKE.ML.CLASSIFICATION](SQL Object Summary/SNOWFLAKE.ML.CLASSIFICATION.md) | ML function | AutoML classification | Training data, feature columns, target | Model and predictions | Table function |
...
|---|---|---|---|---|
| [`DOCUMENT_ID`](Vector Storage Table (RAG Pattern)/DOCUMENT_ID.md) | Identifier | VARCHAR | One per chunk | Business key |
| [`CHUNK_TEXT`](Vector Storage Table (RAG Pattern)/CHUNK_TEXT.md) | Source text | VARCHAR | One per chunk | Raw text segment |
| [`EMBEDDING`](Vector Storage Table (RAG Pattern)/EMBEDDING.md) | Vector | VECTOR(FLOAT, 768) | One per chunk | `EMBED_TEXT_768` output |
| [`METADATA`](Vector Storage Table (RAG Pattern)/METADATA.md) | Context | VARIANT | One per chunk | Source document metadata |
...
| Column | Role | Type | Grain | Notes |
|---|---|---|---|---|
| [`INPUT_ID`](LLM Output Table/INPUT_ID.md) | Identifier | VARCHAR | One per row | Source record |
| [`PROMPT`](LLM Output Table/PROMPT.md) | Input | VARCHAR | One per row | Constructed prompt |
| [`GENERATED_TEXT`](LLM Output Table/GENERATED_TEXT.md) | LLM output | VARCHAR | One per row | `COMPLETE` output |
| [`MODEL_USED`](LLM Output Table/MODEL_USED.md) | Traceability | VARCHAR | One per row | Model name |
| [`TOKENS_USED`](LLM Output Table/TOKENS_USED.md) | Cost metric | NUMBER | One per row | Estimated tokens |
...
|---|---|---|---|---|---|---|
| [`model`](Parameters  Variables  Configuration/model.md) | LLM arg | Model selection | `'mistral-large'`, `'llama3.1-70b'`, `'mixtral-8x7b'`, `'snowflake-arctic'`, etc. | Varies by function | Cortex LLM functions | Determines model capability and cost |
| [`temperature`](Parameters  Variables  Configuration/temperature.md) | LLM arg | Randomness | 0.0 to 1.0 | 0.7 | `COMPLETE` | Controls creativity |
| [`max_tokens`](Parameters  Variables  Configuration/max_tokens.md) | LLM arg | Output limit | Integer > 0 | Model default | `COMPLETE` | Limits generation length |
| [`top_p`](Parameters  Variables  Configuration/top_p.md) | LLM arg | Nucleus sampling | 0.0 to 1.0 | 0.9 | `COMPLETE` | Sampling threshold |
| [`source_language`](Parameters  Variables  Configuration/source_language.md) | TRANSLATE arg | Input language | ISO 639-1 code | Required | `TRANSLATE` | Source language |
| [`target_language`](Parameters  Variables  Configuration/target_language.md) | TRANSLATE arg | Output language | ISO 639-1 code | Required | `TRANSLATE` | Target language |
| [`labels`](Parameters  Variables  Configuration/labels.md) | CLASSIFY_TEXT arg | Candidate classes | ARRAY of strings | Required | `CLASSIFY_TEXT` | Classification categories |
| [`embedding_model`](Parameters  Variables  Configuration/embedding_model.md) | Embedding arg | Embedding model | `'e5-base-v2'`, `'multilingual-e5-large'`, etc. | Required | `EMBED_TEXT_*` | Determines vector semantics |
| [`dimension`](Parameters  Variables  Configuration/dimension.md) | VECTOR type | Vector size | 768, 1024, etc. | Required | Column definition | Fixed array length |
| [`element_type`](Parameters  Variables  Configuration/element_type.md) | VECTOR type | Data type | `FLOAT`, `INT` | `FLOAT` | Column definition | Element data type |
...
|---|---|---|
| [Use Cortex LLM functions](Privilege Requirements/Use Cortex LLM functions.md) | `CORTEX_USER` role or `USAGE` on `SNOWFLAKE.CORTEX` | Database/Schema |
| [Use embedding functions](Privilege Requirements/Use embedding functions.md) | `CORTEX_USER` role or equivalent | Database/Schema |
| [Use vector functions](Privilege Requirements/Use vector functions.md) | `SELECT` on table | Table |
| [Use SNOWFLAKE.ML functions](Privilege Requirements/Use SNOWFLAKE.ML functions.md) | `USAGE` on `SNOWFLAKE.ML` | Schema |
| [Create VECTOR columns](Privilege Requirements/Create VECTOR columns.md) | `CREATE TABLE` on schema | Schema |
