...
|---|---|---|---|---|---|
| SYSTEM$STREAM_HAS_DATA | System function | Test stream for unconsumed deltas | Stream name | String `'TRUE'` or `'FALSE'` | Query or task `WHEN` clause |
| SYSTEM$TASK_DEPENDENTS_ENABLE | System function | Resume task graph | Root task name | Success/failure indicator | `SELECT` or procedure call |
| SYSTEM$GET_PREDECESSOR_RETURN_VALUE | System function | Retrieve predecessor task output | Task name | Predecessor return value string | Inside child task SQL/procedure |
| SYSTEM$CURRENT_USER_TASK_NAME | System function | Identify executing task | None (contextual) | Fully qualified task name or NULL | Inside task SQL/procedure |
| SYSTEM$ABORT_SESSION | System function | Terminate a session | Session ID | Success indicator | `CALL` or direct invocation |
| SYSTEM$CANCEL_QUERY | System function | Cancel a running query | Query ID | Success indicator | `CALL` or direct invocation |
| SYSTEM$TYPEOF | System function | Runtime type introspection | Expression | Type name string | Query-time |
| SYSTEM$WAIT | System function | Procedural delay | Duration in seconds | Success indicator | Inside stored procedure |
| CURRENT_USER / CURRENT_ROLE | Context function | Identity information | Session context | User/role name string | Query-time |
| CURRENT_DATABASE / CURRENT_SCHEMA | Context function | Session context | Session context | Database/schema name string | Query-time |
| CURRENT_TIMESTAMP / CURRENT_DATE | Context function | Temporal context | Session timezone | Timestamp/date value | Query-time |
| CURRENT_VERSION | Context function | Snowflake version | Engine state | Version string | Query-time |
| GET_DDL | Metadata function | Extract object DDL | Object type and name | DDL statement string | Query-time |
...
|---|---|---|---|
| `SYSTEM$STREAM_HAS_DATA` | VARCHAR | Stream monitoring | Returns string `'TRUE'`, not boolean |
| `SYSTEM$TASK_DEPENDENTS_ENABLE` | VARCHAR | Task orchestration | Resumes entire graph, not single task |
| `SYSTEM$GET_PREDECESSOR_RETURN_VALUE` | VARCHAR | Task graph | Returns `NULL` if predecessor failed |
| `SYSTEM$CURRENT_USER_TASK_NAME` | VARCHAR | Task introspection | Returns `NULL` outside task context |
| `SYSTEM$ABORT_SESSION` | VARCHAR | Operational control | Requires elevated privileges |
| `SYSTEM$CANCEL_QUERY` | VARCHAR | Operational control | Query ID must be active |
| `SYSTEM$TYPEOF` | VARCHAR | Debugging | Returns full type spec including precision |
| `SYSTEM$WAIT` | VARCHAR | Procedural | Valid only in stored procedures |
| `CURRENT_USER` | VARCHAR | Identity | Returns current user, not role |
| `CURRENT_ROLE` | VARCHAR | Identity | Returns active role |
| `CURRENT_TIMESTAMP` | TIMESTAMP_LTZ | Temporal | Returns LTZ by default |
| `GET_DDL` | VARCHAR | Metadata | Secure objects return obscured DDL |

## Task Graph State Communication

| Component | Source | Target | Data Type |
|---|---|---|---|
| Predecessor task return value | Predecessor task SQL/procedure | `SYSTEM$GET_PREDECESSOR_RETURN_VALUE` in child | String |
| Task identity | Execution context | `SYSTEM$CURRENT_USER_TASK_NAME` | String |
...
|---|---|---|---|---|---|---|
| Stream name | `SYSTEM$STREAM_HAS_DATA` arg | Target stream | Valid stream name | Required | Task `WHEN`, queries | Determines stream to check |
| Task name | `SYSTEM$TASK_DEPENDENTS_ENABLE` arg | Root task | Valid task name | Required | Orchestration queries | Graph to resume |
| Predecessor name | `SYSTEM$GET_PREDECESSOR_RETURN_VALUE` arg | Source task | Valid predecessor name | Required | Child task body | Task to read return from |
| Session ID | `SYSTEM$ABORT_SESSION` arg | Target session | Valid session ID | Required | Operational queries | Session to terminate |
| Query ID | `SYSTEM$CANCEL_QUERY` arg | Target query | Valid query ID | Required | Operational queries | Query to cancel |
| Expression | `SYSTEM$TYPEOF` arg | Type target | Any valid expression | Required | Debugging queries | Expression to analyze |
| Duration | `SYSTEM$WAIT` arg | Delay length | Number >= 0 | Required | Stored procedures | Seconds to pause |
| `TIMEZONE` | Session parameter | Temporal context | IANA timezone | `UTC` | Session | Affects `CURRENT_TIMESTAMP` |
| `TIMESTAMP_TYPE_MAPPING` | Session parameter | Timestamp type | `TIMESTAMP_LTZ`, `TIMESTAMP_NTZ` | `TIMESTAMP_NTZ` | Session | Affects implicit timestamps |
...
|---|---|---|
| Call `SYSTEM$STREAM_HAS_DATA` | `SELECT` on stream source | Stream |
| Call `SYSTEM$TASK_DEPENDENTS_ENABLE` | `EXECUTE TASK` or `OPERATE` on task | Task |
| Call `SYSTEM$GET_PREDECESSOR_RETURN_VALUE` | Implicit via task execution | Task graph |
| Call `SYSTEM$ABORT_SESSION` | `ACCOUNTADMIN` or `MONITOR` | Account |
| Call `SYSTEM$CANCEL_QUERY` | `ACCOUNTADMIN`, `MONITOR`, or query owner | Query |
| Call `SYSTEM$TYPEOF` | None | N/A |
| Call `SYSTEM$WAIT` | None (procedure context) | N/A |
| Call `GET_DDL` | `USAGE` + `SELECT` or ownership | Target object |
