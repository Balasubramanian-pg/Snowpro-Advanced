...
|---|---|---|---|---|---|
| [COPY INTO](SQL Object Summary/COPY INTO.md) | DML command | Bulk load from stage files to table | Stage files + target table | Loaded rows, error files, load history | Manual SQL or task execution |
| [External Stage](SQL Object Summary/External Stage.md) | Stage object | Reference to cloud storage | S3 bucket, GCS bucket, Azure container | Accessible file paths via `@stage` | `CREATE STAGE` with cloud credentials |
| [Internal Stage](SQL Object Summary/Internal Stage.md) | Stage object | Snowflake-managed storage | User uploads via PUT/Snowsight | File paths via `@~`, `@%table`, `@named_stage` | `CREATE STAGE` or implicit |
| [User Stage](SQL Object Summary/User Stage.md) | Internal stage | Default per-user storage | `PUT` via SnowSQL/Snowsight | `@~/path` | Implicit per user |
| [Table Stage](SQL Object Summary/Table Stage.md) | Internal stage | Table-scoped storage | `PUT` via SnowSQL/Snowsight | `@%table/path` | Implicit per table |
| [Named Stage](SQL Object Summary/Named Stage.md) | Internal/External | Reusable stage object | `CREATE STAGE` definition | `@stage_name/path` | Explicit creation |
| [FILE FORMAT](SQL Object Summary/FILE FORMAT.md) | Schema object | Parsing configuration | Format type + parameters | Reusable format definition | `CREATE FILE FORMAT` |
| [PIPE](SQL Object Summary/PIPE.md) | Schema object | Automated continuous loading | Cloud storage events | Loaded rows in target table | Snowpipe serverless |
| [LOAD_HISTORY](SQL Object Summary/LOAD_HISTORY.md) | System view | Load outcome telemetry | COPY execution | File-level load metrics | Query-time |
...
| [PURGE](SQL Object Summary/PURGE.md) | COPY option | Remove files after load | Stage files | Deleted source files | `COPY INTO` option |
...
| [`ROW_COUNT`](INFORMATION_SCHEMA.LOAD_HISTORY/ROW_COUNT.md) | Loaded volume | One per file per load | Successfully loaded |
| [`ROW_PARSED`](INFORMATION_SCHEMA.LOAD_HISTORY/ROW_PARSED.md) | Parsed volume | One per file per load | Total rows parsed |
| [`ERROR_COUNT`](INFORMATION_SCHEMA.LOAD_HISTORY/ERROR_COUNT.md) | Rejected volume | One per file per load | Rows with errors |
| [`FIRST_ERROR_MESSAGE`](INFORMATION_SCHEMA.LOAD_HISTORY/FIRST_ERROR_MESSAGE.md) | Debug | One per file per load | |
...
| Column | Role | Grain | Notes |
|---|---|---|---|
| [Load-defined columns](Target Table (Load Destination)/Load-defined columns.md) | Data fields | One per row | Must align with COPY projection |
| [`METADATA$FILENAME`](Parameters  Variables  Configuration/METADATA$FILENAME.md) | Source trace | One per row | Available via metadata columns |
| [`METADATA$FILE_ROW_NUMBER`](Parameters  Variables  Configuration/METADATA$FILE_ROW_NUMBER.md) | Source trace | One per row | Row number in source file |
| [`METADATA$FILE_CONTENT_KEY`](Target Table (Load Destination)/METADATA$FILE_CONTENT_KEY.md) | Source trace | One per row | For pipe loads |
...
|---|---|---|---|---|---|---|
| [`ON_ERROR`](Parameters  Variables  Configuration/ON_ERROR.md) | COPY option | Error handling | `ABORT_STATEMENT`, `SKIP_FILE`, `SKIP_FILE_<n>`, `CONTINUE` | `ABORT_STATEMENT` | `COPY INTO` | Determines behavior on bad rows |
| [`FILE_FORMAT`](Parameters  Variables  Configuration/FILE_FORMAT.md) | COPY option | Parsing spec | Named format or inline | Required | `COPY INTO` | Defines how files are parsed |
| [`FILES`](Parameters  Variables  Configuration/FILES.md) | COPY option | Specific files | List of file names | None (all files) | `COPY INTO` | Limits load to named files |
| [`PATTERN`](Parameters  Variables  Configuration/PATTERN.md) | COPY option | Regex filter | Regular expression | None (all files) | `COPY INTO` | Limits load by pattern |
| [`FORCE`](Parameters  Variables  Configuration/FORCE.md) | COPY option | Override dedup | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | `TRUE` risks duplicates |
...
| [`SIZE_LIMIT`](Parameters  Variables  Configuration/SIZE_LIMIT.md) | COPY option | Byte limit | Integer | None | `COPY INTO` | Stops after N bytes |
| [`RETURN_FAILED_ONLY`](Parameters  Variables  Configuration/RETURN_FAILED_ONLY.md) | COPY option | Result filtering | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | Returns only failed files |
| [`VALIDATION_MODE`](Parameters  Variables  Configuration/VALIDATION_MODE.md) | COPY option | Pre-validate | `RETURN_N_ROWS`, `RETURN_ALL_ERRORS` | None | `COPY INTO` | No data persistence |
| [`LOAD_UNCERTAIN_FILES`](Parameters  Variables  Configuration/LOAD_UNCERTAIN_FILES.md) | COPY option | Force load | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | Loads files with uncertain state |
| [`METADATA$FILENAME`](Parameters  Variables  Configuration/METADATA$FILENAME.md) | Metadata column | Source file path | String | Auto | `COPY INTO` column list | Injects source file name |
| [`METADATA$FILE_ROW_NUMBER`](Parameters  Variables  Configuration/METADATA$FILE_ROW_NUMBER.md) | Metadata column | Source row number | Integer | Auto | `COPY INTO` column list | Injects row number |
| [`COMPRESSION`](Parameters  Variables  Configuration/COMPRESSION.md) | File format | Compression type | `AUTO`, `GZIP`, `BZ2`, `DEFLATE`, `RAW_DEFLATE`, `NONE` | `AUTO` | `FILE FORMAT` | Decompression method |
...
| [`RECORD_DELIMITER`](Parameters  Variables  Configuration/RECORD_DELIMITER.md) | File format | Row separator | Character | `\n` | `FILE FORMAT` | Record separator |
| [`FIELD_OPTIONALLY_ENCLOSED_BY`](Parameters  Variables  Configuration/FIELD_OPTIONALLY_ENCLOSED_BY.md) | File format | Quote char | Character | None | `FILE FORMAT` | Quote handling |
| [`ESCAPE`](Parameters  Variables  Configuration/ESCAPE.md) | File format | Escape char | Character | `\\` | `FILE FORMAT` | Escape handling |
...
| [`TIME_FORMAT`](Parameters  Variables  Configuration/TIME_FORMAT.md) | File format | Time parsing | Format string | `AUTO` | `FILE FORMAT` | Time interpretation |
...
| [`BINARY_FORMAT`](Parameters  Variables  Configuration/BINARY_FORMAT.md) | File format | Binary encoding | `HEX`, `BASE64`, `UTF8` | `HEX` | `FILE FORMAT` | Binary interpretation |
| [`ENCODING`](Parameters  Variables  Configuration/ENCODING.md) | File format | Character set | `UTF8`, `ISO2022JP`, `UTF16`, etc. | `UTF8` | `FILE FORMAT` | Character encoding |
...
| [`REPLACE_INVALID_CHARACTERS`](Parameters  Variables  Configuration/REPLACE_INVALID_CHARACTERS.md) | File format | Encoding fix | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Replaces bad chars with replacement char |
...
| [`STRIP_OUTER_ARRAY`](Parameters  Variables  Configuration/STRIP_OUTER_ARRAY.md) | JSON format | Array handling | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Strips outer JSON array |
| [`STRIP_NULL_VALUES`](Parameters  Variables  Configuration/STRIP_NULL_VALUES.md) | JSON format | Null filtering | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Removes null-valued keys |
| [`TRIM_SPACE`](Parameters  Variables  Configuration/TRIM_SPACE.md) | CSV format | Whitespace handling | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Trims whitespace from fields |
...
|---|---|---|
| [Create external stage](Privilege Requirements/Create external stage.md) | `CREATE STAGE` on schema | Schema |
| [Create internal stage](Privilege Requirements/Create internal stage.md) | `CREATE STAGE` on schema | Schema |
| [Use external stage](Privilege Requirements/Use external stage.md) | `USAGE` on stage + storage integration | Stage/Integration |
| [Use internal stage](Privilege Requirements/Use internal stage.md) | `USAGE` on stage | Stage |
| [Load into table](Privilege Requirements/Load into table.md) | `INSERT` on table | Table |
| [Read stage files](Privilege Requirements/Read stage files.md) | `READ` on stage | Stage |
| [Write stage files](Privilege Requirements/Write stage files.md) | `WRITE` on stage | Stage |
