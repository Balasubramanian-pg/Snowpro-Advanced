...
|---|---|---|---|---|---|
| [INSERT](Privilege Requirements/INSERT.md) | DML statement | Add rows to table | VALUES list or SELECT result set | New rows in target table | Manual SQL, task, or application |
| [INSERT INTO ... SELECT](SQL Object Summary/INSERT INTO ... SELECT.md) | DML pattern | Bulk row insertion from query | Source tables/views/subqueries | New rows in target table | Query execution |
| [INSERT OVERWRITE](SQL Object Summary/INSERT OVERWRITE.md) | DML variant | Replace table contents atomically | SELECT result set | Table with only new rows | Atomic truncate + insert |
| [INSERT ALL](SQL Object Summary/INSERT ALL.md) | DML variant | Insert into multiple tables from one source | Source query + multiple target tables | Rows distributed across targets | Single statement |
| [UPDATE](Privilege Requirements/UPDATE.md) | DML statement | Modify existing rows | Target table + SET assignments + WHERE | Updated rows, rewritten micro-partitions | Manual SQL, task, or application |
| [DELETE](Privilege Requirements/DELETE.md) | DML statement | Remove existing rows | Target table + WHERE/USING | Removed rows, rewritten micro-partitions | Manual SQL, task, or application |
| [TRUNCATE TABLE](SQL Object Summary/TRUNCATE TABLE.md) | DDL statement | Efficient full-table deletion | Target table | Empty table, no micro-partition rewrite | Manual SQL |
| [Stream (CDC)](SQL Object Summary/Stream (CDC).md) | Schema object | Capture DML changes | DML on source table | Delta rows with metadata | Automatic |
| [Task](SQL Object Summary/Task.md) | Schema object | Automate DML execution | SQL statement | Scheduled or triggered DML | Time/predecessor based |
...
|---|---|---|---|
| [User-defined columns](Target Table (DML Subject)/User-defined columns.md) | Data | One per row | Subject to DML modification |
| [`METADATA$FILENAME`](Target Table (DML Subject)/METADATA$FILENAME.md) | Metadata | One per row | Available if loaded via COPY |
| [`METADATA$FILE_ROW_NUMBER`](Target Table (DML Subject)/METADATA$FILE_ROW_NUMBER.md) | Metadata | One per row | Available if loaded via COPY |
...
| Column | Role | Notes |
|---|---|---|
| [`TABLE_NAME`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/TABLE_NAME.md) | Identifier | |
| [`TABLE_SCHEMA`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/TABLE_SCHEMA.md) | Context | |
| [`TABLE_TYPE`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/TABLE_TYPE.md) | Classification | `BASE TABLE` for DML targets |
| [`ROW_COUNT`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/ROW_COUNT.md) | Estimate | Approximate; updated periodically |
| [`BYTES`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/BYTES.md) | Storage | Approximate storage size |
| [`RETENTION_TIME`](INFORMATION_SCHEMA.TABLES (DML-Relevant)/RETENTION_TIME.md) | Time Travel | Days of historical retention |

## INFORMATION_SCHEMA.COLUMNS (DML-Relevant)

| Column | Role | Notes |
|---|---|---|
| [`COLUMN_NAME`](INFORMATION_SCHEMA.COLUMNS (DML-Relevant)/COLUMN_NAME.md) | Identifier | |
| [`DATA_TYPE`](INFORMATION_SCHEMA.COLUMNS (DML-Relevant)/DATA_TYPE.md) | Type | Determines coercion rules |
| [`IS_NULLABLE`](INFORMATION_SCHEMA.COLUMNS (DML-Relevant)/IS_NULLABLE.md) | Constraint | `NO` for NOT NULL columns |
| [`COLUMN_DEFAULT`](INFORMATION_SCHEMA.COLUMNS (DML-Relevant)/COLUMN_DEFAULT.md) | Default | Applied when omitted in INSERT |

## INFORMATION_SCHEMA.TABLE_CONSTRAINTS (DML-Relevant)

| Column | Role | Notes |
|---|---|---|
| [`CONSTRAINT_NAME`](INFORMATION_SCHEMA.TABLE_CONSTRAINTS (DML-Relevant)/CONSTRAINT_NAME.md) | Identifier | |
| [`CONSTRAINT_TYPE`](INFORMATION_SCHEMA.TABLE_CONSTRAINTS (DML-Relevant)/CONSTRAINT_TYPE.md) | Classification | `PRIMARY KEY`, `UNIQUE`, `FOREIGN KEY`, `CHECK` |
| [`ENFORCED`](INFORMATION_SCHEMA.TABLE_CONSTRAINTS (DML-Relevant)/ENFORCED.md) | Enforcement | `YES` only for NOT NULL and enabled UNIQUE/PK |

## Stream Delta (if stream exists)

| Column | Role | Notes |
|---|---|---|
| [`METADATA$ACTION`](Stream Delta (if stream exists)/METADATA$ACTION.md) | Delta type | `INSERT` or `DELETE` |
| [`METADATA$ISUPDATE`](Stream Delta (if stream exists)/METADATA$ISUPDATE.md) | Update flag | `TRUE` if update presented as delete+insert |
| [`METADATA$ROW_ID`](Stream Delta (if stream exists)/METADATA$ROW_ID.md) | Delta ID | Unique change identifier |
...
|---|---|---|---|---|---|---|
| [`AUTOCOMMIT`](Parameters  Variables  Configuration/AUTOCOMMIT.md) | Session parameter | Transaction mode | `TRUE`, `FALSE` | `TRUE` | Session | Controls implicit commit per statement |
| [`TIMEZONE`](Parameters  Variables  Configuration/TIMEZONE.md) | Session parameter | Timestamp context | IANA timezone | `UTC` | Session | Affects `CURRENT_TIMESTAMP` in DML |
...
| [`DATA_RETENTION_TIME_IN_DAYS`](Parameters  Variables  Configuration/DATA_RETENTION_TIME_IN_DAYS.md) | Object parameter | Time Travel retention | 0-90 (Enterprise+) | `1` | Table/schema/database | Determines how long old micro-partitions persist |
| [`MAX_DATA_EXTENSION_TIME_IN_DAYS`](Parameters  Variables  Configuration/MAX_DATA_EXTENSION_TIME_IN_DAYS.md) | Account parameter | Fail-safe extension | 0-90 | `0` | Account | Additional recovery beyond Time Travel |
| [`STATEMENT_TIMEOUT_IN_SECONDS`](Parameters  Variables  Configuration/STATEMENT_TIMEOUT_IN_SECONDS.md) | Session parameter | Query abort limit | Integer | `172800` | Session | Cancels long-running DML |
| [`QUERY_TAG`](Parameters  Variables  Configuration/QUERY_TAG.md) | Session parameter | DML traceability | String <= 256 chars | None | Session/query | Tags DML in history views |
| [`ERROR_ON_NONDETERMINISTIC_UPDATE`](Parameters  Variables  Configuration/ERROR_ON_NONDETERMINISTIC_UPDATE.md) | Session parameter | Update safety | `TRUE`, `FALSE` | `FALSE` | Session | Controls error on ambiguous updates |
| [`LOCK_TIMEOUT`](Parameters  Variables  Configuration/LOCK_TIMEOUT.md) | Session parameter | Lock wait limit | Integer (milliseconds) | `43200000` (12 hours) | Session | Max time to wait for lock |
...
|---|---|---|
| [INSERT](Privilege Requirements/INSERT.md) | `INSERT` | Table |
| [UPDATE](Privilege Requirements/UPDATE.md) | `UPDATE` | Table |
| [DELETE](Privilege Requirements/DELETE.md) | `DELETE` | Table |
| [TRUNCATE](Privilege Requirements/TRUNCATE.md) | `DELETE` or `OWNERSHIP` | Table |
| [SELECT (for INSERT...SELECT)](Privilege Requirements/SELECT (for INSERT...SELECT).md) | `SELECT` | Source table |
