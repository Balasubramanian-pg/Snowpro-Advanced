...
|---|---|---|---|---|---|
| COPY INTO | DML command | Bulk load with error handling | Stage files + target table | Loaded rows, error files, result set | Manual SQL, task, or pipe |
| VALIDATION_MODE | COPY option | Pre-validate without loading | Stage files | Error report, no persisted data | `COPY INTO` option |
| ON_ERROR | COPY option | Error behavior control | `ABORT_STATEMENT`, `SKIP_FILE`, `CONTINUE`, `SKIP_FILE_<n>` | Load continuation or abort | `COPY INTO` clause |
| LOAD_HISTORY | System view | File-level load outcomes | COPY execution | Row counts, errors, first error details | Query-time |
| COPY_HISTORY | Account view | Long-term load audit | COPY and pipe operations | File status, rows loaded, errors | Query-time, 45-min latency |
| PIPE_USAGE_HISTORY | Account view | Snowpipe load telemetry | Pipe serverless compute | Files processed, error counts, credits | Query-time, 45-min latency |
| FILE_FORMAT | Schema object | Parsing configuration | Format parameters | Reusable parsing rules | `CREATE FILE FORMAT` |
| Error File | Stage file | Rejected row isolation | COPY `ON_ERROR` | CSV/JSON of bad rows with context | Automatic on `CONTINUE` or `SKIP_FILE` |
| Quarantine Table | User table | Structured bad-row storage | ETL validation logic | Rejected rows with error metadata | DML within procedure |
| TRY_CAST / TRY_TO_DATE | Functions | Safe type conversion | Source string values | Converted value or NULL | Per-row in query |
| INFORMATION_SCHEMA.FILES | System view | Stage file enumeration | Stage contents | File names, sizes, checksums | Query-time |
...
| `ROW_COUNT` | Loaded | One per file per load | Successfully committed |
| `ROW_PARSED` | Parsed | One per file per load | Total rows parsed |
| `ERROR_COUNT` | Rejected | One per file per load | Rows with errors |
| `FIRST_ERROR_MESSAGE` | Debug | One per file per load | First error encountered |
...
| Field | Role | Notes |
|---|---|---|
| Rejected row data | Raw content | Original row as parsed |
| Error code | Classification | Snowflake error code |
| Error message | Detail | Human-readable reason |
| File name | Source | Originating file |
| Line number | Position | Row position in source |

## Quarantine Table (Recommended Pattern)

| Column | Role | Grain | Notes |
|---|---|---|---|
| `QUARANTINE_ID` | Surrogate key | One per row | |
| `SOURCE_FILE` | Provenance | One per row | |
| `REJECTED_ROW` | Raw data | One per row | VARIANT or VARCHAR |
| `ERROR_CODE` | Classification | One per row | |
| `ERROR_MESSAGE` | Detail | One per row | |
| `TARGET_TABLE` | Context | One per row | |
| `LOAD_TIMESTAMP` | Audit | One per row | |
| `RESOLUTION_STATUS` | Tracking | One per row | `OPEN`, `FIXED`, `DISCARDED` |
...
|---|---|---|---|---|---|---|
| `ON_ERROR` | COPY option | Error handling | `ABORT_STATEMENT`, `SKIP_FILE`, `SKIP_FILE_<n>`, `CONTINUE` | `ABORT_STATEMENT` | `COPY INTO` | Determines load behavior on errors |
| `VALIDATION_MODE` | COPY option | Pre-validation | `RETURN_N_ROWS`, `RETURN_ALL_ERRORS` | None | `COPY INTO` | No data persistence |
| `FORCE` | COPY option | Override dedup | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | Reloads already-loaded files |
| `PURGE` | COPY option | Delete after load | `TRUE`, `FALSE` | `FALSE` | `COPY INTO` | Removes source files |
| `FILE_FORMAT` | Format spec | Parsing rules | Named or inline | Required | `COPY INTO` | Defines file parsing |
...
| `ENCODING` | File format | Character set | `UTF8`, etc. | `UTF8` | `FILE FORMAT` | Character encoding |
| `REPLACE_INVALID_CHARACTERS` | File format | Encoding fix | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Replaces bad bytes |
| `COMPRESSION` | File format | Compression | `AUTO`, `GZIP`, etc. | `AUTO` | `FILE FORMAT` | Decompression method |
| `ERROR_ON_NONDETERMINISTIC_MERGE` | Session parameter | Merge safety | `TRUE`, `FALSE` | `FALSE` | Session | Error on ambiguous MERGE |
...
|---|---|---|
| Execute COPY INTO | `INSERT` on table, `USAGE` on stage | Table/Stage |
| Read error files | `READ` on stage | Stage |
| Write quarantine table | `INSERT` on quarantine table | Table |
| Query LOAD_HISTORY | `MONITOR` or `SELECT` | Table/Account |
| Query COPY_HISTORY | `ACCOUNTADMIN` or `MONITOR` | Account |
| Manage pipes | `OWNERSHIP` or `OPERATE` | Pipe |
