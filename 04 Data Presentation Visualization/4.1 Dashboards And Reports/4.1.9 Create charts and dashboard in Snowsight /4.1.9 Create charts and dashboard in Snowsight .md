# 1. Title
Creating Charts and Dashboards in Snowsight: Composition, Configuration, and Consumption Patterns

# 2. Overview
This pattern defines the procedural architecture for designing, building, and deploying interactive charts and dashboards within Snowsight. It exists to enable business stakeholders to consume aggregated insights without writing SQL, reduce redundant query execution through result caching and tile optimization, and maintain a single source of truth for key metrics across an organization. The pattern operates at the consumption layer, bridging curated data models with end-user analytics through a visual composition interface. It is consumed by dashboard authors, analytics engineers building self-service frameworks, business analysts consuming visualized metrics, and SnowPro Advanced candidates evaluating dashboard composition mechanics, filter substitution behavior, result caching boundaries, and sharing permission models.

# 3. SQL Object Summary
| Object/Pattern | Type | Purpose | Source Objects/Inputs | Output Objects/Behavior | Execution Mode |
|----------------|------|---------|------------------------|--------------------------|----------------|
| Snowsight Dashboard Composition | UI Workflow / Query Orchestration Pattern | Assemble parameterized SQL queries into interactive visualizations with filters, layouts, and sharing controls | Curated tables/views, SQL queries with `$FILTER` placeholders, chart specifications, filter definitions | Interactive dashboard with tiles, reusable filters, cached results, shareable URLs | Synchronous (user interaction) with asynchronous tile refresh options |
...
```mermaid
flowchart TD
    A[Curated Data Models] --> B[Snowsight Dashboard Composer]
    B --> C[Tile Registry: SQL + Chart Spec]
    C --> D[Filter Definitions + Bindings]
    D --> E[User Interaction: Filter/Refresh]
...
4. **Execution & Result Caching**
   ...
   - Transformation: Snowflake executes queries with pruning; results cached by query hash + role
...
- **Chart Rendering Limits**: Snowsight renders max 10,000 rows per tile; excess rows truncated with warning. Bar charts recommended for ≤20 categories; scatter plots for ≤10K points; heat grids for ≤15×15 matrices. Aggregation required for high-cardinality source data.
...
| `CHART_TYPE` | Tile Configuration | Select visual encoding for dashboard tile | `BAR`, `COLUMN`, `SCATTER`, `HEAT_GRID`, `SCORECARD`, `LINE`, `AREA` | `BAR` | Tile editor UI | Determines axis mapping options and rendering engine |
...
- **Chart rendering fails due to cardinality limits**: >10K rows sent to browser causes truncation or timeout. Detection: UI warning or blank tile. Recovery: Apply aggregation in tile query; reduce `VISUALIZATION_ROW_LIMIT` or sample data before visualization.
