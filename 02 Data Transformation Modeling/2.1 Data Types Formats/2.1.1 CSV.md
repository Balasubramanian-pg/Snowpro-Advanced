...
|---|---|---|---|---|---|
| FILE_FORMAT (CSV) | Schema object | Parsing configuration for CSV files | CSV parameters | Reusable format definition | `CREATE FILE FORMAT` |
| COPY INTO | DML command | Bulk CSV load | Stage files + target table + FILE_FORMAT | Loaded rows, error files, load history | Manual SQL, task, or pipe |
| VALIDATION_MODE | COPY option | Pre-validate CSV without loading | Stage files + FILE_FORMAT | Error report | `COPY INTO` option |
| ON_ERROR | COPY option | CSV error handling | `ABORT_STATEMENT`, `SKIP_FILE`, `CONTINUE` | Load continuation or abort | `COPY INTO` clause |
| PARSE_HEADER | FILE_FORMAT option | Infer column names from header | `TRUE`, `FALSE` | `FALSE` | `FILE FORMAT` | Uses first row for metadata |
| EXTERNAL TABLE | Schema object | Schema-on-read over CSV files | Cloud storage + FILE_FORMAT | Queryable CSV rows | DDL creation |
| Pipe | Schema object | Automated CSV ingestion | Cloud storage events + FILE_FORMAT | Loaded rows | Snowpipe serverless |
...
|---|---|---|---|
| `TYPE` | Format classification | `CSV` | Must be explicit |
| `FIELD_DELIMITER` | Column separator | `,` | Often changed to `\|` or `\t` |
| `RECORD_DELIMITER` | Row separator | `\n` | Windows files may need `\r\n` awareness |
| `SKIP_HEADER` | Header rows to skip | `0` | Must match actual header count |
| `FIELD_OPTIONALLY_ENCLOSED_BY` | Quote character | None | Usually `"` for RFC 4180 |
| `ESCAPE` | Escape character | `\\` | Set to `NONE` for literal backslashes |
| `NULL_IF` | Null indicators | `['\\N', 'NULL']` | Exam trap: `'NULL'` string becomes NULL |
| `ERROR_ON_COLUMN_COUNT_MISMATCH` | Strict columns | `TRUE` | `FALSE` allows ragged rows |
| `TRIM_SPACE` | Whitespace handling | `FALSE` | `TRUE` trims all string fields |
| `REPLACE_INVALID_CHARACTERS` | Encoding fix | `FALSE` | `TRUE` replaces bad bytes |
| `ENCODING` | Character set | `UTF8` | Match source encoding |
| `COMPRESSION` | Compression | `AUTO` | Detect or specify |
| `DATE_FORMAT` | Date parsing | `AUTO` | Specify for non-standard dates |
| `TIME_FORMAT` | Time parsing | `AUTO` | Specify for non-standard times |
| `TIMESTAMP_FORMAT` | Timestamp parsing | `AUTO` | Specify for non-standard timestamps |
| `BINARY_FORMAT` | Binary encoding | `HEX` | `BASE64` or `UTF8` alternatives |
| `PARSE_HEADER` | Schema inference | `FALSE` | `TRUE` for external table column detection |
...
| `ROW_COUNT` | Loaded | One per file | Successfully loaded rows |
| `ROW_PARSED` | Parsed | One per file | Total rows parsed |
| `ERROR_COUNT` | Rejected | One per file | Rows with errors |
| `FIRST_ERROR_MESSAGE` | Debug | One per file | First parse/type error |
...
|---|---|---|---|---|---|---|
| `TYPE` | FILE_FORMAT | Format type | `CSV` | Required | `CREATE FILE FORMAT` | Selects CSV parser |
| `FIELD_DELIMITER` | FILE_FORMAT | Column separator | Character(s) | `,` | `CREATE FILE FORMAT` | Splits fields |
| `RECORD_DELIMITER` | FILE_FORMAT | Row separator | Character(s) | `\n` | `CREATE FILE FORMAT` | Splits records |
| `SKIP_HEADER` | FILE_FORMAT | Header rows | Integer >= 0 | `0` | `CREATE FILE FORMAT` | Rows ignored at start |
| `FIELD_OPTIONALLY_ENCLOSED_BY` | FILE_FORMAT | Quote char | Character | None | `CREATE FILE FORMAT` | Handles quoted fields |
| `ESCAPE` | FILE_FORMAT | Escape char | Character | `\\` | `CREATE FILE FORMAT` | Escape handling |
| `ESCAPE_UNENCLOSED_FIELD` | FILE_FORMAT | Unquoted escape | Character | `\\` | `CREATE FILE FORMAT` | Escape for unquoted fields |
| `NULL_IF` | FILE_FORMAT | Null values | List of strings | `['\\N', 'NULL']` | `CREATE FILE FORMAT` | Strings → SQL NULL |
| `ERROR_ON_COLUMN_COUNT_MISMATCH` | FILE_FORMAT | Strict columns | `TRUE`, `FALSE` | `TRUE` | `CREATE FILE FORMAT` | Rejects ragged rows |
| `TRIM_SPACE` | FILE_FORMAT | Whitespace trim | `TRUE`, `FALSE` | `FALSE` | `CREATE FILE FORMAT` | Trims field whitespace |
| `REPLACE_INVALID_CHARACTERS` | FILE_FORMAT | Encoding fix | `TRUE`, `FALSE` | `FALSE` | `CREATE FILE FORMAT` | Replaces bad bytes |
| `ENCODING` | FILE_FORMAT | Character set | `UTF8`, `ISO2022JP`, `UTF16`, etc. | `UTF8` | `CREATE FILE FORMAT` | Byte-to-char conversion |
| `COMPRESSION` | FILE_FORMAT | Compression | `AUTO`, `GZIP`, `BZ2`, `DEFLATE`, `RAW_DEFLATE`, `NONE` | `AUTO` | `CREATE FILE FORMAT` | Decompression method |
| `DATE_FORMAT` | FILE_FORMAT | Date parsing | Format string | `AUTO` | `CREATE FILE FORMAT` | Date interpretation |
| `TIME_FORMAT` | FILE_FORMAT | Time parsing | Format string | `AUTO` | `CREATE FILE FORMAT` | Time interpretation |
| `TIMESTAMP_FORMAT` | FILE_FORMAT | Timestamp parsing | Format string | `AUTO` | `CREATE FILE FORMAT` | Timestamp interpretation |
| `BINARY_FORMAT` | FILE_FORMAT | Binary encoding | `HEX`, `BASE64`, `UTF8` | `HEX` | `CREATE FILE FORMAT` | Binary data format |
| `PARSE_HEADER` | FILE_FORMAT | Schema inference | `TRUE`, `FALSE` | `FALSE` | `CREATE FILE FORMAT` | Extracts column names |
| `ON_ERROR` | COPY option | Error handling | `ABORT_STATEMENT`, `SKIP_FILE`, `CONTINUE` | `ABORT_STATEMENT` | `COPY INTO` | Load behavior on errors |
| `VALIDATION_MODE` | COPY option | Pre-validation | `RETURN_N_ROWS`, `RETURN_ALL_ERRORS` | None | `COPY INTO` | No data persistence |
...
| Load CSV | `INSERT` on table, `READ` on stage | Table/Stage |
| Preview CSV | `SELECT` on stage | Stage |
